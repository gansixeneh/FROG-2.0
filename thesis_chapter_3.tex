...
Figure~\ref{fig:dsr-methodology} illustrates the six main activities of the Design Science Research methodology that guide our research approach. The methodology comprises a systematic process that begins with problem identification and progresses through solution design, development, demonstration, evaluation, and communication phases.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{dsr.png}
    \caption{Design Science Research (DSR) Methodology Framework showing the six main activities and their relationships}
    \label{fig:dsr-methodology}
\end{figure}

As shown in Figure~\ref{fig:dsr-methodology}, the DSR methodology follows a structured approach that ensures rigorous development and evaluation of our enhanced GraphRAG system. Each phase builds upon the previous one, creating a comprehensive framework for addressing the identified research challenges. The following is the explanation of each activity, in relation to our work.
...
\label{exp-detail}
In this section, we present the experimental setup for evaluating our enhanced GraphRAG system against the original baseline. We begin by describing the original pipeline architecture that serves as our baseline, followed by an overview of the enhancements we propose to address its limitations.

\subsection{Original Pipeline (Baseline)}
\label{subsec:original-pipeline}

The original FrOG (Framework of Open GraphRAG) system was developed through two pipeline iterations, each addressing different aspects of the knowledge graph question answering challenge. We describe both pipelines in detail as they collectively form our baseline for comparison.

\subsubsection{Pipeline v1: Initial Prototype}

The first pipeline, illustrated in Figure~\ref{fig:pipeline_v1}, was designed as a prototype specifically focused on Wikidata. This initial implementation established the foundational approach for translating natural language questions into SPARQL queries.
...
\begin{enumerate}
    \item \textbf{Entity Extraction and Linking}: Identifies entities mentioned in the natural language question and links them to corresponding Wikidata entities using the Wikidata API.
    
    \item \textbf{SPARQL Query Generation}: Generates SPARQL queries using few-shot prompting with Large Language Models. The property candidates are directly included in the generation prompt, obtained from the top-100 frequently used properties listed on Wikidata's website\footnote{\url{https://www.wikidata.org/wiki/Wikidata:Database_reports/List_of_properties/Top100}}.
    
    \item \textbf{Answer Generation}: Executes the generated SPARQL query against Wikidata Query Service and formats the results into a natural language response.
\end{enumerate}

While this prototype demonstrated the feasibility of the approach, it had significant limitations. The reliance on a predefined list of only 100 properties severely restricted the types of questions the system could handle. Additionally, the narrow focus on Wikidata meant the approach could not be easily adapted to other knowledge graphs with different schemas and property structures.

\subsubsection{Pipeline v2: Enhanced Architecture}

Recognizing the limitations of the initial prototype, the second pipeline was developed to address the property limitation and enable multi-knowledge graph support. Figure~\ref{fig:pipeline_v2} illustrates this enhanced architecture.
...
The main improvements in Pipeline v2 include:

\begin{enumerate}
    \item \textbf{Vector Database Integration}: Incorporation of auxiliary vector databases to store and retrieve a much larger number of properties beyond the predefined top-100 list, enabling the system to handle more diverse question types.
    
    \item \textbf{Dual Processing Approach}: Introduction of a verbalization path for simple (single-hop) questions, where the system first attempts to answer using direct property retrieval before falling back to SPARQL generation. This approach uses dense text embeddings encoded from verbalized triples to measure semantic similarity with the user's question.
    
    \item \textbf{Multi-Knowledge Graph Support}: Extension to support multiple knowledge bases including Wikidata, DBpedia, and local knowledge graphs, demonstrating the framework's flexibility across different domains.
    
    \item \textbf{Enhanced Retrieval Stage}: The system implements a more sophisticated retrieval process where:
    \begin{itemize}
        \item Entities are retrieved from the knowledge graph's API (if available) or vector database
        \item Properties are retrieved from the vector database based on semantic similarity with the question
        \item The retrieval results provide better context for SPARQL generation
    \end{itemize}
\end{enumerate}

\subsubsection{Dataset Generation for Local Knowledge Graphs}

A notable contribution of the original framework was the development of an automatic dataset generation system for local knowledge graphs where no existing evaluation datasets exist. This system supports the generation of four types of questions, as described in Table~\ref{table-questiontype-original}.

\begin{table}[htbp]
\centering
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{12pt}
\caption{Supported Question Categories in Original Dataset Generator}
\begin{tabular}{|c | c|}
\hline
...
\hline
\end{tabular}
\label{table-questiontype-original}
\end{table}

The dataset generation process follows five main steps:

\begin{enumerate}
    \item \textbf{Entity Selection}: Randomly selects one entity from the knowledge graph
    \item \textbf{Random Walk}: Performs a random walk based on the question category to discover valid query patterns
    \item \textbf{Resource Label Resolution}: Resolves natural language labels for properties and entities
    \item \textbf{SPARQL Query Formation}: Constructs SPARQL queries using predefined templates
    \item \textbf{Question Generation}: Uses LLMs to generate natural language questions from the SPARQL queries
\end{enumerate}

This dataset generation approach enabled evaluation on local knowledge graphs by creating synthetic question-SPARQL pairs. However, the generated datasets had limitations in terms of linguistic diversity and the quality of the generated natural language questions.

\subsubsection{Limitations of the Original Pipeline}

Despite the improvements in Pipeline v2, several fundamental limitations remained that motivated our enhanced approach:

\begin{enumerate}
    \item \textbf{Limited Dataset Quality and Diversity}: The dataset generation relied on simple random walks and basic LLM prompting, resulting in:
    \begin{itemize}
        \item Limited linguistic diversity in generated questions
        \item Lack of comprehensive coverage of possible query patterns
        \item No systematic approach to ensure balanced complexity distribution
        \item Absence of chain-of-thought reasoning in the datasets
    \end{itemize}
    
    \item \textbf{Heavy Reliance on Few-Shot Prompting}: The system's dependence on few-shot prompting for both entity/property extraction and SPARQL generation led to:
    \begin{itemize}
        \item Inconsistent performance across different question types
        \item Inability to learn domain-specific patterns effectively
        \item High sensitivity to the quality and selection of few-shot examples
        \item Limited capability to handle complex multi-hop queries
    \end{itemize}
    
    \item \textbf{Rigid and Black-Box Architecture}: The pipeline implementation suffered from:
    \begin{itemize}
        \item Fixed processing paths with limited adaptability
        \item Lack of comprehensive fallback mechanisms beyond basic verbalization
        \item No visibility into the reasoning process or decision-making
        \item Absence of logging and visualization capabilities for debugging
        \item Limited runtime configurability for different use cases
    \end{itemize}
\end{enumerate}

These limitations highlighted the need for a more sophisticated approach that could provide better dataset generation, specialized model capabilities through fine-tuning, and a flexible, transparent architecture with comprehensive fallback mechanisms.

\subsection{Overall Enhancement Strategy}
\label{subsec:enhancement-strategy}

To address the limitations identified in the original pipeline, we propose a comprehensive enhancement strategy consisting of three main improvements that work synergistically to create a more robust and capable GraphRAG system. Figure~\ref{fig:enhancement-overview} illustrates these three core components of our enhancement strategy: dataset generation, model fine-tuning, and multi-agent architecture.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{assets/pics/enhancement.png}
    \caption{Three core components of the enhanced GraphRAG system: (1) Dataset Generation creates high-quality training data through template-based and random-walk approaches, (2) Model Fine-tuning specializes LLMs for entity/property extraction and SPARQL generation tasks, and (3) Multi-Agent Architecture orchestrates specialized agents with intelligent routing and comprehensive fallback mechanisms}
    \label{fig:enhancement-overview}
\end{figure}

As shown in Figure~\ref{fig:enhancement-overview}, our enhancement strategy addresses the original system's limitations through a coordinated approach that improves data quality, model capabilities, and system architecture. Each component plays a crucial role in creating a more capable and transparent GraphRAG system.

\subsubsection{Enhanced Dataset Generation}

We develop sophisticated dataset generation methods that significantly improve upon the original approach:

\begin{itemize}
    \item \textbf{Dual Generation Approaches}: We implement both template-based and random-walk generation methods:
    \begin{itemize}
        \item Template-based generation ensures linguistic diversity and high-quality natural language questions
        \item Random-walk generation discovers comprehensive graph patterns beyond predefined templates
    \end{itemize}
    
    \item \textbf{Chain-of-Thought Integration}: All generated datasets include step-by-step reasoning chains that explain the translation from natural language to SPARQL, providing valuable training signals for models
    
    \item \textbf{Systematic Complexity Distribution}: Questions are stratified across three complexity levels (basic, intermediate, advanced) to ensure balanced training data
    
    \item \textbf{SPARQL Post-Processing}: Generated queries undergo comprehensive formatting to align with natural language model generation patterns
\end{itemize}

\subsubsection{Specialized Model Fine-tuning}

Instead of relying on few-shot prompting, we implement parameter-efficient fine-tuning to create specialized models:

\begin{itemize}
    \item \textbf{Entity and Property Extraction Model}: Fine-tuned specifically for identifying entities and properties in natural language questions, providing more consistent and accurate extraction
    
    \item \textbf{SPARQL Generation Model}: Fine-tuned on our enhanced datasets to generate syntactically correct and semantically appropriate SPARQL queries with chain-of-thought reasoning
    
    \item \textbf{Domain-Specific Adaptation}: Models are fine-tuned separately for each knowledge graph domain to capture domain-specific patterns and terminology
    
    \item \textbf{QLoRA Implementation}: We use Quantized Low-Rank Adaptation to enable efficient fine-tuning on standard hardware while maintaining model quality
\end{itemize}

\subsubsection{Multi-Agent Architecture with LangGraph}

We replace the rigid pipeline with a sophisticated multi-agent system that provides:

\begin{itemize}
    \item \textbf{Intelligent Routing}: Adaptive processing paths based on query complexity and runtime settings, with specialized agents for different tasks
    
    \item \textbf{Comprehensive Fallback Mechanisms}: Multiple layers of fallback including verbalization to SPARQL and knowledge graph to web search
    
    \item \textbf{Complete Transparency}: Detailed logging and visualization of the entire reasoning process through:
    \begin{itemize}
        \item RDF-based semantic logging for SPARQL-queryable execution traces
        \item Real-time process visualization with Mermaid diagrams
        \item JSON export of complete execution logs
    \end{itemize}
    
    \item \textbf{Runtime Configurability}: Flexible settings for enabling/disabling features, selecting knowledge sources, and controlling processing strategies
\end{itemize}

These three enhancements, as depicted in Figure~\ref{fig:enhancement-overview}, work together to create a system that is more accurate, transparent, and adaptable than the original pipeline, while maintaining the commitment to using only open-source components. The following sections detail the implementation of each enhancement component.

% [Continue with the rest of the chapter as in the original...]
% \section{Knowledge Bases} \label{knowledgebase}
% [Rest of the chapter continues unchanged...]

% =============================================================================

\section{Knowledge Bases} \label{knowledgebase}

\subsection{Rationale for Multi-Domain Evaluation}

The decision to evaluate our enhanced GraphRAG system across multiple knowledge graphs is driven by several critical research considerations:

\begin{enumerate}
    \item \textbf{Generalizability Validation}: A key research question is whether our enhanced methodology (dataset generation, model finetuning, and multi-agent architecture) can generalize beyond a single knowledge domain. Testing on only one KG would limit our ability to claim that our approach is domain-agnostic and broadly applicable.
    
    \item \textbf{Diverse Structural Challenges}: Different knowledge graphs present fundamentally different structural patterns and querying challenges:
    \begin{itemize}
        \item \textbf{Scale variations}: From small domain-specific KGs (874 triples) to massive general-purpose KGs (billions of triples)
        \item \textbf{Schema complexity}: From simple flat structures to complex hierarchical ontologies
        \item \textbf{Query patterns}: From basic property lookups to complex multi-hop reasoning with aggregations
        \item \textbf{Language characteristics}: From general natural language to domain-specific terminology
    \end{itemize}
    
    \item \textbf{Methodological Robustness}: By testing across diverse domains, we can identify which aspects of our methodology are universally effective and which require domain-specific adaptations, providing valuable insights for future GraphRAG research.
\end{enumerate}

\subsubsection{Selection Criteria for Knowledge Graphs}

The four specific knowledge graphs were selected based on the following criteria:

\begin{enumerate}
    \item \textbf{Wikidata}: Selected as the benchmark large-scale KG because:
    \begin{itemize}
        \item It serves as the de facto standard for GraphRAG evaluation with established datasets (QALD-9-Plus)
        \item Its massive scale (117M+ entities) tests our system's scalability
        \item It provides a baseline for comparison with existing GraphRAG systems
    \end{itemize}
    
    \item \textbf{Curriculum KG}: Selected to represent educational/organizational domains because:
    \begin{itemize}
        \item It demonstrates our ability to work with locally-constructed, small-scale KGs
        \item The structured nature of curriculum data provides clear evaluation criteria
        \item It represents a common use case for institutions wanting to query their own data
    \end{itemize}
    
    \item \textbf{Legal KG}: Selected to represent document-centric domains because:
    \begin{itemize}
        \item Legal reasoning requires precise query formulation and complex relationships
        \item The hierarchical structure of legal documents tests multi-hop reasoning
        \item It demonstrates applicability to governmental and regulatory use cases
    \end{itemize}
    
    \item \textbf{GESIS KG}: Selected to represent scholarly/research domains because:
    \begin{itemize}
        \item It tests our system's ability to handle bibliographic and citation networks
        \item The rich metadata structure challenges our entity/property extraction
        \item It represents the growing need for scientific literature analysis
    \end{itemize}
\end{enumerate}

\subsubsection{Data Availability and Access}

All knowledge graphs used in our evaluation are based on openly available data sources:

\begin{itemize}
    \item \textbf{Wikidata}: Fully open access via public SPARQL endpoint and APIs
    \item \textbf{Curriculum KG}: Constructed from publicly available curriculum documents (accessible via Fasilkom UI's LMS)
    \item \textbf{Legal KG}: Based on the open Lex2KG dataset of Indonesian legal documents
    \item \textbf{GESIS KG}: Publicly accessible research metadata
\end{itemize}

This ensures our experiments are fully reproducible and our methodology can be validated by other researchers without requiring special access permissions.

Having established the rationale for multi-domain evaluation, we now describe each knowledge graph in detail. These four knowledge bases—spanning general knowledge, educational curricula, legal documents, and scholarly research—collectively provide a comprehensive testbed for evaluating our enhanced GraphRAG methodology.

As discussed in our selection criteria, each knowledge base was specifically chosen to test different aspects of our system. Below, we provide detailed technical descriptions of their structure, content, and the specific preprocessing steps we applied to ensure data quality.

Having established the rationale for multi-domain evaluation, we now describe each knowledge graph in detail. These four knowledge bases\footnote{Originally, we planned to include DBpedia as a fifth evaluation knowledge base. However, during our implementation phase, we discovered that the DBpedia Lookup service (\url{https://lookup.dbpedia.org/}) was unavailable, preventing us from implementing the necessary entity retrieval functionality. While the service became operational again on June 6, 2025, we had already completed our evaluation framework focusing on the four selected knowledge bases.}---spanning general knowledge, educational curricula, legal documents, and scholarly research---collectively provide a comprehensive testbed for evaluating our enhanced GraphRAG methodology.
...
\subsection{Chain-of-Thought Reasoning: A Core Design Principle}
\label{cot-principle}

A fundamental design decision in our dataset generation methodology is the systematic incorporation of chain-of-thought (CoT) reasoning---referred to as ``thoughts" throughout this work. This decision stems from extensive preliminary experimentation that revealed thoughts are not merely a performance optimization but an essential component for building effective and trustworthy GraphRAG systems.

Our methodology incorporates thoughts for several critical reasons:

\begin{enumerate}
    \item \textbf{Bridging the Semantic Gap}: Natural language questions and SPARQL queries represent fundamentally different ways of expressing information needs. Thoughts provide explicit intermediate steps that bridge this semantic gap, making the transformation process learnable by models. For example, transforming ``Who succeeded Kennedy as president?" into a complex SPARQL query with qualifier properties requires multiple reasoning steps that would be opaque without explicit thoughts.
    
    \item \textbf{Improving Model Accuracy}: Our preliminary experiments showed that models trained with thoughts achieved significantly higher accuracy compared to direct question-to-SPARQL mapping. The structured reasoning process guides models through the logical steps required for correct query construction, particularly crucial for complex multi-hop queries.
    
    \item \textbf{Providing Training Signals}: Thoughts serve as crucial intermediate supervision signals during model training. They help models learn how to decompose complex questions into manageable sub-tasks, identify relevant entities and properties for different question types, construct appropriate SPARQL patterns, and determine when to use specific SPARQL features (e.g., qualifiers, filters, aggregations). This scaffolding effect is particularly important for questions requiring multiple reasoning steps, where direct supervision would be insufficient.
    
    \item \textbf{Enabling System Transparency}: In knowledge-intensive domains like legal and medical information retrieval, understanding how an answer was derived is as important as the answer itself. Thoughts make the reasoning process explicit and auditable, allowing users to verify the system's logic.
    
    \item \textbf{Facilitating Debugging and Error Analysis}: When queries produce incorrect results, thoughts provide a clear trace of the reasoning process. Developers can identify exactly where the logic failed---whether in entity recognition, property selection, or query structure formulation---significantly reducing debugging time.
    
    \item \textbf{Supporting Multi-Stakeholder Needs}: Different stakeholders benefit from thoughts in distinct ways:
    \begin{itemize}
        \item \textit{End users} gain transparency into how answers are derived
        \item \textit{Developers} can debug and improve the system more effectively
        \item \textit{Domain experts} can validate reasoning against domain knowledge
        \item \textit{Researchers} can analyze reasoning patterns and failure modes
    \end{itemize}
\end{enumerate}

Our methodology ensures consistent thought integration across all dataset generation approaches. For QALD-9-Plus Enhancement (Wikidata), we use Large Language Models to generate step-by-step reasoning chains for existing question-SPARQL pairs. In template-based generation, thoughts are embedded directly in templates as parameterized reasoning chains. For random-walk generation, we use LLMs to create contextually appropriate thoughts that explain the reasoning behind each discovered query pattern.

The importance of thoughts extends throughout our entire system architecture. Fine-tuned models are trained to generate thoughts alongside SPARQL queries, providing real-time reasoning transparency. The multi-agent architecture captures and logs thoughts during query processing (see Section~\ref{vis-transparency}), enabling comprehensive analysis of the reasoning process. This integration ensures that the benefits of chain-of-thought reasoning---improved accuracy, transparency, and debuggability---are realized throughout the entire GraphRAG pipeline, from dataset creation through model training to end-user interaction.
...
For Curriculum KG, Legal KG, and GESIS KG, where no existing evaluation datasets exist, we generate complete datasets from scratch using template-based and random-walk approaches with complexity stratification, as detailed in the following sections. Figure~\ref{fig:dataset-generation-overview} illustrates our dual dataset generation methodology that combines these approaches.
...
While the template-based approach provides high-quality, linguistically diverse questions, it may not capture the full range of structural patterns present in a knowledge graph. To address this limitation, we developed a pattern-based random walk methodology that discovers valid query patterns directly from the graph structure, as illustrated in Figure~\ref{fig:random-walk-workflow}.
\begin{figure}
    \centering
    ...
    \label{fig:random-walk-workflow}
\end{figure}
...
\begin{enumerate}
    ...
    \end{itemize}
    Figure~\ref{fig:1-property} visualizes this basic pattern structure.
    ...
    \end{itemize}
    Figure~\ref{fig:2-property} illustrates these intermediate pattern structures.
    ...
    \end{itemize}
    These advanced patterns are visualized in Figure~\ref{fig:3-property}.
...
Figure~\ref{fig:sparql-postprocessing} illustrates the complete four-stage transformation pipeline that standardizes SPARQL queries across all our datasets, ensuring consistent formatting that aligns with the natural generation patterns of language models.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{assets/pics/post-new.jpg}
...
To overcome limitations in previous approaches that relied on few-shot prompting, we formulated a specialized model fine-tuning methodology using Low-Rank Adaptation (LoRA) for two critical components: entity and property extraction, and SPARQL query generation. This methodological approach significantly improves performance over zero-shot or few-shot approaches by tailoring model weights specifically for knowledge graph tasks while maintaining computational efficiency. Figure~\ref{fig:lora-architecture} illustrates the QLoRA architecture employed in our methodology, which dramatically reduces trainable parameters while maintaining model performance. We designed distinct hyperparameter configurations for each task based on their complexity and the nature of the required outputs.

\begin{figure}[htbp]
    \centering
        \includegraphics[width=1\textwidth]{assets/pics/qlora.jpg}
...
This configuration approach results in approximately 40-60 million trainable parameters (depending on model size), representing less than 1\% of the original model parameters, enabling efficient adaptation while capturing domain-specific extraction patterns. Figure~\ref{fig:parameter-efficiency} demonstrates the dramatic reduction in trainable parameters achieved through this LoRA approach compared to full model fine-tuning.
...
We incorporated advanced methodological optimizations including gradient computation strategies and batch organization techniques to handle the longer sequences involved in SPARQL generation training. Figure~\ref{fig:training-process} illustrates our complete training methodology, from data preparation through model selection.

\begin{figure}[htbp]
    \centering
        \includegraphics[width=1\linewidth]{assets/pics/training.jpg}
...
A key innovation in our fine-tuning methodology is the cross-domain adaptation approach, which enables efficient development across different knowledge graph domains. We formulated a sequential experimental approach to optimize our methodology, as illustrated in Figure~\ref{fig:cross-domain}:
...
To enable adaptation of large language models on standard research infrastructure, we developed a comprehensive methodology for computational efficiency, as shown in Figure~\ref{fig:memory-optimization}:

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{assets/pics/eff.png}
...
This methodological approach to computational efficiency enables adaptation of large models (7-12 billion parameters) on standard research infrastructure, making our approach accessible and reproducible for researchers with limited computational resources. Figure~\ref{fig:learning-curves} demonstrates typical learning progression patterns during model adaptation, showing training and validation loss convergence behavior.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=1\linewidth]{assets/pics/learning2.jpg}
...
\subsection{Iterative Prompt Engineering and Format Optimization}
\label{iterative-prompt}

Throughout the development of our finetuning methodology, we conducted systematic experiments to optimize the prompt structure and SPARQL formatting for maximum model performance. This iterative approach revealed critical insights about how formatting decisions and prompt engineering significantly impact the quality of generated SPARQL queries.

\subsubsection{Experimental Progression}

Our optimization process followed an incremental approach, with each experiment building upon insights from previous iterations:

\begin{enumerate}
    \item \textbf{Baseline Configuration}: Initially, we used minimal post-processing with raw SPARQL queries presented in compact format without thoughts or formatting enhancements. For example:
    \begin{lstlisting}[language=SPARQL, caption=Baseline SPARQL format without post-processing]
SELECT DISTINCT ?uri WHERE { ?uri wdt:P31 wd:Q194188 ; wdt:P137 wd:Q23548 . }
\end{lstlisting}
    This baseline configuration established our performance floor for comparison.

    \item \textbf{Indentation Enhancement}: We introduced structured indentation to improve query readability and align with common SPARQL formatting practices:
    \begin{lstlisting}[language=SPARQL, caption=SPARQL with indentation formatting]
SELECT DISTINCT ?uri
WHERE {
  ?uri wdt:P31 wd:Q194188 ;
       wdt:P137 wd:Q23548 .
}
\end{lstlisting}
    This modification resulted in a measurable increase in Jaccard similarity scores compared to the baseline, suggesting that models benefit from structured formatting during training.

    \item \textbf{Whitespace Normalization}: Through analysis of model outputs, we discovered that fine-tuned models consistently generated queries with specific whitespace patterns, particularly removing spaces before question marks and dots. We adapted our training data to match these patterns:
    \begin{lstlisting}[language=SPARQL, caption=SPARQL with whitespace normalization]
SELECT DISTINCT?uri WHERE { VALUES?types { wd:Q3918 wd:Q875538 } wd:Q567 wdt:P69?uri.?uri wdt:P31?types. }
\end{lstlisting}
    This alignment with the model's natural generation tendencies produced additional performance improvements.

    \item \textbf{Combined Formatting}: We integrated both indentation and whitespace normalization to leverage the benefits of both approaches:
    \begin{lstlisting}[language=SPARQL, caption=SPARQL with combined formatting optimizations]
SELECT DISTINCT?uri
WHERE {
...
\end{lstlisting}
    This combined approach yielded higher performance gains than either technique in isolation.

    \item \textbf{Chain-of-Thought Integration}: The most significant improvement came from incorporating explicit reasoning steps before SPARQL generation, as detailed in Section~\ref{cot-principle}. As explained in our dataset generation methodology, thoughts serve multiple critical purposes including accuracy enhancement, debugging facilitation, and system transparency. In our experiments, this approach yielded the most substantial performance gains:
    \begin{lstlisting}[language=, caption=Example of chain-of-thought reasoning followed by SPARQL generation, basicstyle=\small\ttfamily]
Let me analyze this step by step:
1. The question asks for the person who succeeded John F. Kennedy as President of the United States.
2. From the provided URIs, 'Q9696' represents John F. Kennedy and 'Q11696' represents the position of President of the United States.
3. The property 'P39' represents 'position held', and 'P1366' represents 'replaced by', which together can identify Kennedy's successor.
4. I need to find the entity that replaced Kennedy in the position of US President.
5. The SPARQL query should identify the statement where Kennedy held the position of US President and then retrieve the entity specified as his replacement.
...
```
    ...
    The inclusion of thoughts aligned with our core design principle (Section~\ref{cot-principle}) and resulted in the highest accuracy improvements across all experimental configurations.
\end{enumerate}

\subsubsection{Performance Impact}

Each iterative improvement contributed to enhanced model performance, with the final configuration incorporating all optimizations achieving the highest Jaccard similarity scores. The progression of improvements can be summarized as follows:

\begin{itemize}
    \item \textbf{Baseline → Indentation}: Moderate improvement in query generation accuracy
    \item \textbf{Baseline → Whitespace Normalization}: Similar moderate improvement
    \item \textbf{Baseline → Combined Formatting}: Larger improvement than individual techniques
    \item \textbf{Combined Formatting → Chain-of-Thought}: Substantial improvement, achieving the best overall performance
\end{itemize}

The dramatic improvement from incorporating chain-of-thought reasoning validates our design decision to make thoughts a core component of our dataset generation methodology (Section~\ref{cot-principle}). This experimental validation, combined with the broader benefits of transparency and debugging described earlier, informed our decision to systematically incorporate thoughts throughout our entire pipeline.

\subsubsection{Final Prompt Configuration}

Based on these experiments, our final prompt configuration incorporates all discovered optimizations. The system prompt emphasizes structured reasoning and precise formatting requirements, while the assistant prompt template enforces the chain-of-thought pattern followed by properly formatted SPARQL. This configuration, which operationalizes the chain-of-thought principle described in Section~\ref{cot-principle}, represents the culmination of our iterative optimization process and is used throughout all finetuning experiments reported in this thesis.

These iterative experiments demonstrate the critical importance of prompt engineering and format optimization in achieving high-quality SPARQL generation. The insights gained from this process informed not only our Wikidata experiments but also transferred effectively to domain-specific knowledge graphs, validating the generalizability of our optimization approach.
...
\label{result-evaluation-methodology}
The most critical aspect of our evaluation is the assessment of query results. We use the Jaccard similarity coefficient to compare the results returned by generated queries against those from ground truth queries. This metric measures the overall similarity between two result sets by calculating the size of the intersection divided by the size of the union of the sets.
...
This stratification allows us to identify specific strengths and weaknesses of our system and to assess the effectiveness of our fine-tuning approach across different complexity levels (for domain-specific KGs) and knowledge domains. Figure~\ref{fig:comparative-eval} illustrates our comparative evaluation methodology for assessing base and fine-tuned models across different dataset types and metrics.
