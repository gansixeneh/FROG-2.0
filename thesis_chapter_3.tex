...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/pdfs/pipeline-ori.pdf}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.8\textwidth]{assets/pdfs/pipeline2-cropped.pdf}
    \caption{The Upgraded Pipeline (Pipeline v2) \citep{ongris2024frog}}
    \label{fig:pipeline_v2}
\end{figure}

However, both original pipelines are limited by their simplified decision-making architecture, lack of specialized model training, and constrained dataset generation capabilities. These limitations highlight the need for our enhanced approach.
...
Finally, we conduct our evaluation using Kaggle's cloud-based infrastructure \footnote{\url{http://kaggle.com/}} to ensure computational scalability and reproducibility of our enhanced approach compared to the original baseline systems.
...
\begin{enumerate}
    ...
    \item \textbf{Entity Enhancement}: We extract entities from both questions and SPARQL queries, then use the Wikidata API to retrieve matching entity URIs and labels. We intentionally add noise entities to improve model robustness during training.
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{assets/pics/dataset1.png}
...
\begin{lstlisting}[
  language=json, 
  caption=Example of a Generated Dataset Entry, 
...
{
  "id": "q1",
  "question": "How many credits does the Algorithm Design 
    and Analysis course have?",
  "sparql": "SELECT ?value WHERE { 
    <http://example.org/algorithm_design_and_analysis> 
    <http://example.org/has_credits> ?value . }",
  "category": "university",
  "complexity": "basic",
  "templateId": "course-credits",
  "thoughts": [
    "1. The question asks for the credit value of 
    Algorithm Design and Analysis.",
    "2. The entity 'Algorithm Design and Analysis' 
    represents a course in the university domain.",
    "3. The property 'has_credits' links a course 
    to its credit value.",
    "4. To solve this, retrieve the credit value 
    linked to Algorithm Design and Analysis via 
    the 'has_credits' property.",
    "5. Construct a SPARQL query to retrieve the 
    credit value for Algorithm Design and Analysis."
  ],
...
\begin{figure}
    \centering
    \includegraphics[width=0.75\linewidth]{assets/pics/template.png}
...
\begin{figure}
    \centering
    \includegraphics[width=0.5\linewidth]{assets/pics/salib1.jpg}
...
\begin{figure}[htbp]
    ...
    \caption{LoRA Architecture: Only small adapter modules are trained while the base model remains frozen, reducing trainable parameters by over 99\% compared to full fine-tuning}
...
For the entity and property extraction task, we designed a lighter LoRA configuration since this task requires less adaptation from the base model's capabilities:
...
\begin{itemize}
    ...
    \item \textbf{LoRA configuration}: Standard approach without rank stabilization, as extraction tasks showed less susceptibility to training instabilities
\end{itemize}
...
For SPARQL generation, we designed a more intensive adaptation configuration due to the task's complexity and the need for precise syntax generation:

\begin{itemize}
    \item \textbf{Rank (r)}: 64 - Higher rank to provide increased capacity for learning complex syntax patterns
    \item \textbf{Alpha ($\alpha$)}: 64 - Matched to the rank parameter for consistent adaptation
    ...
    \item \textbf{LoRA configuration}: Rank-Stabilized approach to prevent training instabilities
\end{itemize}
...
\label{cross-domain}
A key innovation in our fine-tuning methodology is the cross-domain adaptation approach, which enables efficient knowledge transfer between different knowledge graph domains. Rather than developing separate models for each domain, we formulated a sequential adaptation methodology:
...
\begin{enumerate}
    \item \textbf{Base adaptation}: Initial adaptation using the Wikidata domain, which contains the most diverse entity and property types
    \item \textbf{Domain adaptation}: Further adaptation to domain-specific patterns using the Wikidata-adapted model as the foundation
\end{enumerate}

This methodological approach significantly reduces computational requirements while enabling models to leverage general knowledge graph reasoning capabilities across domains. The key domain adaptations include:

\begin{itemize}
    \item \textbf{Domain-specific prefixes}: Adaptation of URI patterns and namespace conventions
    \item \textbf{Schema awareness}: Integration of domain-specific ontology structures
    \item \textbf{Query patterns}: Learning of domain-specific query formulations
\end{itemize}
...
\begin{itemize}
    ...
    \item \textbf{Parameter-Efficient Adaptation}: Application of LoRA to reduce trainable parameters to less than 3\% of the full model
...
\begin{enumerate}
    ...
    \item \textbf{Entity Extraction Agent}: Analyzes the (translated) natural language question to identify relevant entities and properties mentioned. This agent leverages a fine-tuned language model specifically trained for entity and property extraction tasks as described in Section~\ref{entity-prop-extract}.
    ...
    \item \textbf{SPARQL Generation Agent}: Generates executable SPARQL queries for complex questions using a fine-tuned language model specifically trained on NL2SPARQL tasks as described in Section~\ref{sparql-gen}. This agent can handle multi-hop reasoning and complex query patterns.
