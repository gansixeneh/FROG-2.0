...
def replace_uri_with_prefix(sparql_query):
    """Replace URIs in the SPARQL query with appropriate prefixes."""
    # Replace schema.org URIs with schema prefix
    sparql_query = re.sub(r'<https://schema\.org/([^>]+)>', r'schema:\1', sparql_query)
    
    # Replace GESIS KG URIs with gesiskg prefix
    sparql_query = re.sub(r'<https://data\.gesis\.org/gesiskg/schema/([^>]+)>', r'gesiskg:\1', sparql_query)
...
def main():
    # Load the JSON file
    with open('gesis_dataset.json', 'r') as f:
    ...
    # Save training data
    train_file = 'gesis_train.json'
    with open(train_file, 'w') as f:
        json.dump(train_data, f, indent=2)
    
    # Save test data
    test_file = 'gesis_test.json'
