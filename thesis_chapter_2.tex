...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{assets/pics/qlora.png}
    \caption{QLoRA architecture: The base model weights are quantized to 4-bit precision (shown in blue), while only the adapter modules (low-rank matrices $A$ and $B$) remain in higher precision for training (shown in orange).}
    \label{fig:qlora-concept}
\end{figure}

As illustrated in \pic~\ref{fig:qlora-concept}, QLoRA introduces several key innovations beyond standard LoRA:
...
\begin{enumerate}
    ...
    \item \textbf{Paged Optimizers}: To manage memory more efficiently during training, QLoRA implements paged optimizers that offload optimizer states to CPU memory when not in use and fetch them back to GPU memory as needed.
