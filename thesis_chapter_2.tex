...
\begin{figure}
\centering
\includegraphics[width=0.8\linewidth]{assets/pics/tree.png}
...
\begin{enumerate}
    ...
    \item \textbf{Single-Agent Architecture}: The pipeline architecture, while straightforward, lacked the sophisticated reasoning capabilities that a multi-agent approach could provide for adaptive strategy selection and intelligent query routing.
    ...
    \item \textbf{Sequential Processing}: The rigid pipeline architecture provided limited opportunities for feedback loops, error recovery, or alternative processing paths when primary approaches failed.
...
\begin{enumerate}
    ...
    \item \textbf{Single-Agent Limitations}: Many current GraphRAG systems employ single-agent or simple pipeline architectures that lack the sophisticated reasoning capabilities, adaptive processing, and robust fallback mechanisms provided by multi-agent approaches.
