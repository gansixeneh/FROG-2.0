...
class EntityPropertyRetrieval:
    def __init__(self, weaviate_url, weaviate_api_key, 
        ...
        if entity_count == 0:
            ...
            print(f"Found {entity_count} entities in Weaviate. Skipping entity loading.")
            
        if property_count == 0:
            self.load_properties_if_needed()
        else:
            print(f"Found {property_count} properties in Weaviate. Skipping property loading.")
    ...
    def _get_entity_count(self):
        ...
        try:
            result = self.client.query.aggregate("WikidataEntity").with_meta_count().do()
    ...
    def _get_property_count(self):
        ...
        try:
            result = self.client.query.aggregate("WikidataProperty").with_meta_count().do()
    ...
    def load_entities_if_needed(self, batch_limit=1000):
        """
        Load all Wikidata entities into Weaviate.
        ...
        # Use offset pagination to load all entities
        offset = 0
        total_loaded = 0
        ...
        while True:
            ...
            if df.empty:
                print(f"No more entities found or reached API limit. Total loaded: {total_loaded}")
                break
            
            # Clean entity IDs to extract Q numbers
            df['entity_id'] = df['entity'].str.extract(r'(Q\d+)')
            
            # Generate embeddings and add to Weaviate in batches
            with self.client.batch as batch:
                batch.batch_size = self.batch_size
                
                for i, row in tqdm(df.iterrows(), total=len(df), desc=f"Adding entities (batch {offset//batch_limit + 1})"):
                    # Skip if missing data
                    if pd.isna(row['entityLabel']) or pd.isna(row['entity_id']):
                        continue
                    
                    # Create text for embedding
                    text = f"{row['entityLabel']}"
                    if not pd.isna(row['entityDescription']):
                        text += f": {row['entityDescription']}"
                    ...
                    entity_object = {
                        ...
                        "description": row['entityDescription'] if not pd.isna(row['entityDescription']) else ""
                    }
            ...
            total_loaded += len(df)
            print(f"Added batch of {len(df)} entities. Total so far: {total_loaded}")
    ...
    def load_properties_if_needed(self, batch_limit=1000):
        """
        Load all Wikidata properties into Weaviate.
        ...
        """
        print("Loading Wikidata properties...")
        
        # Use offset pagination to load all properties
        offset = 0
        total_loaded = 0
        ...
        while True:
            ...
            df = self.query_engine.run_query(query)
            if df.empty:
                print(f"No more properties found or reached API limit. Total loaded: {total_loaded}")
                break
            ...
            with self.client.batch as batch:
                ...
                for i, row in tqdm(df.iterrows(), total=len(df), desc=f"Adding properties (batch {offset//batch_limit + 1})"):
                    # Skip if missing data
                    if pd.isna(row['propertyLabel']) or pd.isna(row['property_id']):
                        continue
                    
                    # Create text for embedding
                    text = f"{row['propertyLabel']}"
                    if not pd.isna(row['propertyDescription']):
                        text += f": {row['propertyDescription']}"
                    ...
                    property_object = {
                        ...
                        "description": row['propertyDescription'] if not pd.isna(row['propertyDescription']) else ""
                    }
            ...
            total_loaded += len(df)
            print(f"Added batch of {len(df)} properties. Total so far: {total_loaded}")
    ...
    def search_entities(self, query_text, limit=10):
        ...
        result = (
            self.client.query
            .get("WikidataEntity", ["entity_id", "label", "description"])
        ...
        if "data" in result and "Get" in result["data"] and "WikidataEntity" in result["data"]["Get"]:
    ...
    def search_properties(self, query_text, limit=10):
        ...
        result = (
            self.client.query
            .get("WikidataProperty", ["property_id", "label", "description"])
            .with_near_vector({"vector": query_embedding})
            .with_limit(limit)
            .do()
        )
        ...
        # Convert to DataFrame
        if "data" in result and "Get" in result["data"] and "WikidataProperty" in result["data"]["Get"]:
...
if __name__ == '__main__':
    ...
    retriever = EntityPropertyRetrieval(
        weaviate_url=os.getenv("WEAVIATE_URL"),  # e.g., "https://your-cluster-id.weaviate.cloud"
    ...
    # Search for entities
    entity_results = retriever.search_entities("scientist who developed theory of relativity")
