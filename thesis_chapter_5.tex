...
We conduct several experiments to evaluate the performance of our enhanced GraphRAG system against the baseline system from the previous work, FrOG. We assess the system holistically using Jaccard Similarity, as explained in Section \ref{result-evaluation-methodology}. Additionally, we evaluate the effectiveness of our fine-tuning approach by comparing our system with base models against our system with fine-tuned counterparts. It is important to note that while FrOG's original evaluation in their paper used full models, our evaluation (including our reimplementation of FrOG) uses quantized models due to resource constraints. Furthermore, FrOG relies on few-shot examples for its operation, while our system---both with base and fine-tuned models---does not use any few-shot prompting. The information provided in this section is organized based on the knowledge graph domain.

\subsection{Wikidata}
\label{wikidata-results}

Using the QALD-9-Plus dataset for Wikidata, we evaluate the performance of both the baseline system (FrOG) from the previous work and our enhanced system with both base and fine-tuned models. It is important to note that while FrOG's original evaluation in their paper was conducted on a downsampled QALD-9-Plus train dataset, our evaluation (including our reimplementation of FrOG) is performed on the QALD-9-Plus test dataset for a fair comparison. Table \ref{table-wikidata-results} presents the comparison between FrOG results and our current results with both base and fine-tuned models across different model architectures.
...
The results demonstrate several important findings. First, all of our fine-tuned models outperform their base model counterparts. Most of our fine-tuned models also outperform FrOG, despite the fact that FrOG leverages few-shot examples while our system does not use any few-shot prompting. The most significant improvement is observed with the Llama-3.1-8B model, which shows a dramatic increase from a near-zero base model performance ($0.0079$) to achieving the highest overall score ($0.3237$) after fine-tuning. This represents an absolute gain of $0.0353$ over the FrOG implementation.

Similarly, the Mistral-Nemo model shows substantial improvement from $0.0000$ in its base form to $0.3024$ after fine-tuning, surpassing its performance in FrOG ($0.2884$). The Qwen2.5-7B model achieves the second-highest overall performance ($0.3176$), slightly improving upon its FrOG result ($0.3150$).
...
\begin{lstlisting}[language=SPARQL, caption=Our Fine-tuned Model Output for "Who is the mayor of Berlin?", label=code:berlin-mayor-finetuned]
SELECT DISTINCT ?uri
WHERE {
  wd:Q64 wdt:P6 ?uri.
}
...
In contrast, the FrOG implementation produced an incorrect query, despite leveraging few-shot examples:
...
\begin{lstlisting}[language=SPARQL, caption=Our Fine-tuned Model Output for EU Countries with Euro, label=code:eu-euro-finetuned]
SELECT DISTINCT ?uri
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Our Fine-tuned Model Output for Vietnam War Presidents, label=code:vietnam-finetuned]
SELECT DISTINCT ?uri
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Our Fine-tuned Model Output for Constitutional Monarchies, label=code:monarchy-finetuned]
SELECT DISTINCT ?uri
WHERE {
...
For the Curriculum Knowledge Graph, we evaluate performance using both template-based and random-walk datasets. It is important to note that FrOG was originally evaluated on Curriculum KG in their paper, so they had established few-shot examples for this knowledge graph. In our evaluation, we implemented FrOG using only the Qwen/Qwen2.5-7B-Instruct model due to time constraints, and we maintained their few-shot approach. Table \ref{table-curriculum-template-results} presents the results on the template dataset, while Table \ref{table-curriculum-random-results} shows the results on the random-walk dataset.
...
The results on the Curriculum Knowledge Graph demonstrate substantial improvements over the FrOG implementation across all models, even though FrOG benefits from few-shot examples while our system does not. Several key observations can be made:

First, our fine-tuned models significantly outperform both FrOG and our base models, with absolute improvements over FrOG ranging from $0.0536$ (Qwen2.5-7B fine-tuned with template dataset on template dataset) to $0.0959$ (Qwen2.5-7B fine-tuned with template dataset on random-walk dataset). This substantial improvement highlights the effectiveness of our fine-tuning approach for domain-specific knowledge graphs with well-defined schema structures.
...
Third, the Qwen2.5-Coder-7B model achieves the highest performance on the template dataset ($0.7042$) when fine-tuned with template data, while Llama-3.1-8B shows strong performance when fine-tuned on random-walk data for the template dataset ($0.7131$). These results suggest different strengths among model architectures when handling curriculum knowledge structures.
...
\begin{lstlisting}[language=SPARQL, caption=Our Template Fine-tuned Model Output for Alternative Course Name, label=code:curriculum-alt-finetuned]
SELECT ?alt_name
WHERE {
...
Both implementations correctly identify the essential structure of the query, but our fine-tuned model demonstrates improved variable naming conventions that align better with standard SPARQL practices. Notably, our model achieves this without relying on few-shot examples, unlike FrOG.
...
\begin{lstlisting}[language=SPARQL, caption=Our Random-Walk Fine-tuned Model Output for Course Category, label=code:curriculum-category-finetuned]
SELECT ?category
WHERE {
...
Again, both implementations correctly capture the essential query structure, but our fine-tuned model produces cleaner variable naming and more consistent formatting without requiring few-shot examples.
...
\begin{lstlisting}[language=SPARQL, caption=Our Base Model Output for Research Lab with Most Courses, label=code:curriculum-lab-base]
...
\begin{lstlisting}[language=SPARQL, caption=Our Template Fine-tuned Model Output for Research Lab with Most Courses, label=code:curriculum-lab-template]
SELECT ?group
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Our Random-Walk Fine-tuned Model Output for Research Lab with Most Courses, label=code:curriculum-lab-random]
SELECT ?target
WHERE {
...
\begin{enumerate}
    ...
    \item Our fine-tuning approach eliminates the need for few-shot examples, potentially making the system more adaptable to varied query formats.
\end{enumerate}

Overall, these results demonstrate that our fine-tuning approach is particularly effective for domain-specific knowledge graphs with well-defined structures, outperforming the few-shot approach used by FrOG.

\subsection{Legal Knowledge Graph}
\label{legal-results}

For the Legal Knowledge Graph, we also evaluate performance using both template-based and random-walk datasets. It is important to note that FrOG was not originally evaluated on the Legal KG in their paper, so we had to create few-shot examples from our own training datasets. As with Curriculum KG, we implemented FrOG using only the Qwen/Qwen2.5-7B-Instruct model due to time constraints. Table \ref{table-legal-template-results} presents the results on the template dataset, while Table \ref{table-legal-random-results} shows the results on the random-walk dataset.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \RaggedRight{\textbf{Model}} & \textbf{FrOG} & \textbf{Base Model} & \textbf{Fine-tuned Template} & \textbf{Fine-tuned Random-Walk} \\
    \hline
    \RaggedRight{Qwen/Qwen2.5-7B-Instruct} & $0.3487$ & $0.2872$ & $\textbf{0.4731}$ & $0.3653$ \\
...
\begin{adjustbox}{width=\textwidth}
    ...
    \RaggedRight{\textbf{Model}} & \textbf{FrOG} & \textbf{Base Model} & \textbf{Fine-tuned Template} & \textbf{Fine-tuned Random-Walk} \\
    \hline
    \RaggedRight{Qwen/Qwen2.5-7B-Instruct} & $0.3642$ & $0.2692$ & $\textbf{0.4357}$ & $0.4257$ \\
...
The results on the Legal Knowledge Graph show substantial improvements over both FrOG and our base models. Key observations include:

First, our fine-tuned models significantly outperform FrOG, with absolute improvements ranging from $0.0715$ (Qwen2.5-7B fine-tuned with template dataset on random-walk dataset) to $0.1244$ (Qwen2.5-7B fine-tuned with template dataset on template dataset). These improvements highlight the effectiveness of our fine-tuning approach for the legal domain, which features complex document structures and relationships, even without the benefit of few-shot examples that FrOG utilizes.
...
\begin{lstlisting}[language=SPARQL, caption=Our Template Fine-tuned Model Output for Article References, label=code:legal-ref-finetuned]
...
Our fine-tuned model correctly uses the \texttt{lex2kg-o:merujuk} property to identify references to the specified article, and appropriately connects the text segments to their containing articles with the \texttt{lex2kg-o:versi} property. The FrOG implementation incorrectly attempts to find articles with the exact text ``UU tahun 2017 no 18 pasal 68" or directly references the URI, which is less effective in the legal knowledge graph structure, despite having the advantage of few-shot examples.
...
\begin{lstlisting}[language=SPARQL, caption=Our Random-Walk Fine-tuned Model Output for Article References, label=code:legal-ref2-finetuned]
SELECT ?pasal WHERE {
  ?pasal lex2kg-o:merujuk <https://example.org/lex2kg/uu/1950/1/pasal/0072>.
}
\end{lstlisting}

\begin{lstlisting}[language=SPARQL, caption=Our Base Model Output for Article References, label=code:legal-ref2-base]
SELECT ?uu
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Our Base Model Output for Complex Article Query, label=code:legal-complex-base]
SELECT ?pasal
WHERE {
  ?uu a lex2kg:UU;
     lex2kg-o:nomor "1953";
...
\begin{lstlisting}[language=SPARQL, caption=Our Template Fine-tuned Model Output for Complex Article Query, label=code:legal-complex-template]
SELECT ?pasal
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Our Random-Walk Fine-tuned Model Output for Complex Article Query, label=code:legal-complex-random]
SELECT ?pasal WHERE {
  ?pasal rdf:type lex2kg-o:Pasal.
...
\begin{enumerate}
    ...
    \item Our approach eliminates the dependency on manually crafted few-shot examples, which is particularly valuable for domains like the legal knowledge graph where expertise is required to create effective examples.
\end{enumerate}

\subsection{GESIS Knowledge Graph}
\label{gesis-results}

For the GESIS Knowledge Graph, we again evaluate performance using both template-based and random-walk datasets. Similar to the Legal KG, FrOG was not originally evaluated on GESIS KG in their paper, so we created few-shot examples from our own training datasets. As with the previous knowledge graphs, we implemented FrOG using only the Qwen/Qwen2.5-7B-Instruct model due to time constraints. Table \ref{table-gesis-template-results} presents the results on the template dataset, while Table \ref{table-gesis-random-results} shows the results on the random-walk dataset.
...
The results on the GESIS Knowledge Graph demonstrate the largest improvements over the FrOG implementation among all knowledge graph domains, despite FrOG's advantage of using few-shot examples. Key observations include:

First, our fine-tuned models significantly outperform FrOG, with absolute improvements ranging from $0.1208$ (Qwen2.5-7B fine-tuned with template dataset on random-walk dataset) to $0.1896$ (Qwen2.5-7B fine-tuned with template dataset on template dataset). These substantial improvements highlight the effectiveness of our fine-tuning approach for scholarly knowledge graphs with standardized metadata schema, even without the benefit of few-shot examples.
...
\begin{lstlisting}[language=SPARQL, caption=Our Template Fine-tuned Model Output for Publications Count, label=code:gesis-pub-finetuned]
...
\begin{lstlisting}[language=SPARQL, caption=Our Base Model Output for Publications Count, label=code:gesis-pub-base]
SELECT (COUNT(?publication) AS ?numberOfPublications)
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Our Random-Walk Fine-tuned Model Output for Author Works, label=code:gesis-author-finetuned]
...
\begin{lstlisting}[language=SPARQL, caption=Our Base Model Output for Author Works, label=code:gesis-author-base]
SELECT (COUNT(?work) AS ?numberOfWorks)
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Our Template Fine-tuned Model Output for Gleichstellung Resources, label=code:gesis-resources-template]
...
\begin{lstlisting}[language=SPARQL, caption=Our Random-Walk Fine-tuned Model Output for Gleichstellung Resources, label=code:gesis-resources-random]
SELECT DISTINCT ?resource
WHERE {
...
\begin{enumerate}
    ...
    \item Our approach eliminates the dependency on manually crafted few-shot examples, which proves especially effective for standardized domains like scholarly metadata.
\end{enumerate}

\section{General Analysis}
In this section, we will provide a general analysis of the experiment results from the perspectives of the large language models (LLMs), knowledge graph characteristics, fine-tuning approach, dataset generation methods, and system performance comparison.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Model}} & \textbf{Parameters} & \textbf{Our System (Base Model Avg)} & \textbf{Our System (Fine-tuned Avg)} & \textbf{Improvement} & \textbf{Training Tokens} \\
    \hline
    \RaggedRight{Qwen/Qwen2.5-7B-Instruct} & 7B & $0.3143$ & $0.4756$ & $+0.1613$ & 18T \\
    \RaggedRight{Qwen/Qwen2.5-Coder-7B-Instruct} & 7B & $0.3299$ & $\textbf{0.4812}$ & $+0.1513$ & 5.2T (70\% code) \\
    \RaggedRight{meta-llama/Llama-3.1-8B-Instruct} & 8B & $0.2621$ & $0.4557$ & $+0.1936$ & 15T \\
    \RaggedRight{mistralai/Mistral-Nemo-Instruct-2407} & 12B & $0.2316$ & $0.4708$ & $\textbf{+0.2392}$ & Undisclosed \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
...
First, the Qwen2.5-Coder-7B model achieves the highest average performance after fine-tuning ($0.4812$), leveraging its code-heavy pre-training (70\% coding-related content) to excel at structured query generation tasks. The model demonstrates consistent performance across all knowledge graph domains.

Second, the Qwen2.5-Coder-7B model also shows the highest base performance ($0.3299$) before fine-tuning, suggesting that its specialized training provides initial advantages for SPARQL generation. However, it shows the smallest improvement from fine-tuning ($+0.1513$), potentially because its specialized training already incorporates some of the reasoning patterns needed for structured queries.

Third, the Mistral-Nemo-12B model shows the largest relative improvement from fine-tuning ($+0.2392$), despite starting from the lowest base performance ($0.2316$). This suggests exceptional adaptability to the specific requirements of knowledge graph query generation through our fine-tuning methodology.

Fourth, the Llama-3.1-8B model shows substantial improvement ($+0.1936$) and notably achieves the best fine-tuned performance on Wikidata ($0.3237$), demonstrating effective adaptation to complex query patterns when dealing with numeric identifiers.
...
\begin{enumerate}
    ...
    \item \textbf{Adaptability}: Models show different levels of adaptability to fine-tuning, with Mistral-Nemo-12B demonstrating the highest relative improvement despite not achieving the best overall performance.
    
    \item \textbf{Few-shot vs. Fine-tuning}: Our results demonstrate that fine-tuning consistently outperforms the few-shot approach used by FrOG across all knowledge graph domains, suggesting that the comprehensive learning enabled by fine-tuning is more effective than the pattern-matching facilitated by few-shot examples.
\end{enumerate}

These findings suggest that for knowledge graph applications, the combination of diverse pre-training and effective fine-tuning is more important than raw parameter count or few-shot capabilities. The Qwen2.5-7B model's strong performance across all knowledge graph domains makes it our recommended model for general knowledge graph applications, while specialized variants like Qwen2.5-Coder-7B may offer advantages in specific scenarios where base model performance without fine-tuning is prioritized.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Knowledge Graph}} & \textbf{Size (Triples)} & \textbf{URI Scheme} & \textbf{Our System (Base Model Avg)} & \textbf{Our System (Fine-tuned Avg)} & \textbf{Improvement} \\
    \hline
    \RaggedRight{Wikidata} & 117M entities & Numeric IDs & $0.1158$ & $0.3101$ & $+0.1943$ \\
    \RaggedRight{Curriculum KG} & 874 triples & Natural language & $0.4948$ & $\textbf{0.6111}$ & $+0.1163$ \\
    \RaggedRight{Legal KG} & 1.1M triples & Mixed & $0.2676$ & $0.4216$ & $+0.1539$ \\
    \RaggedRight{GESIS KG} & 1.9M resources & Natural language & $0.2597$ & $0.4798$ & $\textbf{+0.2201}$ \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
...
The manner in which resources are represented using URIs significantly influences model performance. Knowledge graphs with natural language-based URIs, such as the Curriculum KG (\texttt{ns1:algorithm\_design\_and\_analysis}) and GESIS KG (\texttt{schema:ScholarlyArticle}), enable models to achieve substantially higher performance. Both our system with fine-tuned models and our system with base models perform better on these knowledge graphs compared to those with numeric or opaque identifiers.

Wikidata, which uses numeric IDs for both entities (\texttt{wd:Q64} for Berlin) and properties (\texttt{wdt:P36} for capital), presents the greatest challenge for models. The lack of semantic information in these identifiers requires models to learn arbitrary associations between natural language expressions and numeric codes, making the task inherently more difficult. This is particularly challenging for our system with base models ($0.1158$), while fine-tuning helps bridge this gap significantly ($0.3101$).
...
The size and homogeneity of knowledge graphs also significantly impact performance. The Curriculum KG, despite being the smallest with only 874 triples, achieves the highest average performance after fine-tuning ($0.6111$). Its small size, well-defined schema, and domain-specific focus create a more manageable learning environment for models.

In contrast, Wikidata's massive scale (over 117 million entities) and extraordinarily diverse domain coverage present significant challenges, resulting in the lowest performance across all models ($0.3101$ after fine-tuning). The sheer breadth of entities, properties, and relationships makes it difficult for models to learn comprehensive patterns, even with few-shot examples as used by FrOG.

The GESIS and Legal knowledge graphs, with intermediate sizes (1-2 million triples) and domain-specific focus, achieve moderate performance. However, the GESIS KG shows the largest improvement from fine-tuning ($+0.2201$), suggesting that its standardized schema based on schema.org vocabularies may be particularly amenable to learning through our fine-tuning approach. The Legal KG shows a moderate improvement ($+0.1539$), reflecting the challenges posed by its mixed URI scheme.
...
\begin{enumerate}
    ...
    \item \textbf{Scale Challenges}: Very large knowledge graphs present substantial challenges for query generation, even with fine-tuning or few-shot examples. Domain-specific subsets or specialized views may be more practical for high-performance applications.
    ...
    \item \textbf{Fine-tuning vs. Few-shot}: The relative advantage of our fine-tuning approach over FrOG's few-shot approach appears greater for knowledge graphs with standardized schemas (GESIS) than for those with idiosyncratic structures (Wikidata), suggesting that fine-tuning is particularly valuable for learning standardized patterns.
\end{enumerate}
...
Our experiment results demonstrate the substantial impact of our fine-tuning methodology across all models and knowledge graphs. Since Wikidata only has a single dataset type (based on QALD-9-Plus), we present its results separately in Table \ref{table-wikidata-finetune}, while Table \ref{table-finetune-comparison} compares the two fine-tuning approaches (template-based and random-walk-based) for the other knowledge graph domains.

\begin{table}
\centering
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{12pt}
\caption{Fine-tuning Results on Wikidata}
\label{table-wikidata-finetune}
\begin{adjustbox}{width=\textwidth}
    \begin{tabular}{| m{0.4\textwidth} | m{0.2\textwidth} | m{0.2\textwidth} | m{0.2\textwidth} |}
    \hline
    \RaggedRight{\textbf{Model}} & \textbf{Base Model} & \textbf{Fine-tuned} & \textbf{Improvement} \\
    \hline
    \RaggedRight{Qwen/Qwen2.5-7B-Instruct} & $0.2021$ & $0.3176$ & $+0.1155$ \\
    \RaggedRight{Qwen/Qwen2.5-Coder-7B-Instruct} & $0.2531$ & $0.2967$ & $+0.0436$ \\
    \RaggedRight{meta-llama/Llama-3.1-8B-Instruct} & $0.0079$ & $0.3237$ & $+0.3158$ \\
    \RaggedRight{mistralai/Mistral-Nemo-Instruct-2407} & $0.0000$ & $0.3024$ & $+0.3024$ \\
    \hline
    \RaggedRight{\textbf{Average}} & $\textbf{0.1158}$ & $\textbf{0.3101}$ & $\textbf{+0.1943}$ \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
...
\renewcommand{\arraystretch}{1.15}
\setlength{\tabcolsep}{12pt}
\caption{Fine-tuning Approach Comparison for Knowledge Graphs with Multiple Dataset Types}
...
\begin{adjustbox}{width=\textwidth}
    ...
    \RaggedRight{Legal KG - Template Dataset} & $0.2731$ & $\textbf{0.4539}$ & $0.3929$ & Template \\
    \RaggedRight{Legal KG - Random-Walk Dataset} & $0.2622$ & $\textbf{0.4257}$ & $0.4138$ & Template \\
    \RaggedRight{GESIS KG - Template Dataset} & $0.2679$ & $\textbf{0.5118}$ & $0.4445$ & Template \\
...
First, for Wikidata, fine-tuning shows dramatic improvements across all models, with particularly striking gains for Llama-3.1-8B ($+0.3158$) and Mistral-Nemo ($+0.3024$), which started from near-zero base performance. The average improvement of $+0.1943$ demonstrates the critical importance of fine-tuning for knowledge graphs with numeric identifiers, where models must learn arbitrary associations between natural language and opaque entity/property codes. Notably, these improvements are achieved without requiring few-shot examples, in contrast to FrOG.

Second, for knowledge graphs with both template and random-walk dataset types, both fine-tuning approaches substantially improve performance over base models. However, template-based fine-tuning consistently outperforms random-walk-based fine-tuning across all knowledge graphs and dataset types. This somewhat surprising result suggests that the structured, pattern-focused nature of template-based training provides advantages even when evaluating on random-walk-generated test sets. The linguistic diversity and structured patterns in template-based training appear to enable better generalization to diverse query types.

Third, the advantage of template-based fine-tuning is most pronounced on the template datasets, with significant margins on knowledge graphs like Legal KG (template: $0.4539$ vs random-walk: $0.3929$) and GESIS KG (template: $0.5118$ vs random-walk: $0.4445$). Even on random-walk datasets, template-based fine-tuning maintains its advantage, though with smaller margins.

Fourth, fine-tuning effectiveness varies across knowledge graph types. The GESIS Knowledge Graph shows the largest average improvement from fine-tuning (approximately $+0.22$), while the Curriculum Knowledge Graph shows the smallest improvement (approximately $+0.11$) despite achieving the highest absolute performance. This suggests that knowledge graphs with standardized schemas like GESIS may benefit more from structured fine-tuning approaches, while Wikidata's unique challenges require specialized handling.

These findings lead to several important insights about our fine-tuning methodology:

\begin{enumerate}
    \item \textbf{Universal Effectiveness}: Fine-tuning provides substantial improvements across all knowledge graph types, from the massive and diverse Wikidata to domain-specific graphs like GESIS. This universal effectiveness validates our approach as a general solution for knowledge graph query generation.
    
    \item \textbf{Template Superiority}: For knowledge graphs with multiple dataset types, the superiority of template-based fine-tuning suggests that focusing on common, linguistically diverse query patterns provides better learning signals than the more random, structure-focused patterns from random walks. This may be because templates better align with how humans naturally formulate questions about knowledge graphs.
    ...
    \item \textbf{Advantage over Few-shot}: Our fine-tuning approach consistently outperforms FrOG's few-shot approach across all knowledge graphs, even on Wikidata where FrOG was originally designed and optimized. This suggests that the comprehensive learning enabled by fine-tuning is more effective than the pattern-matching facilitated by few-shot examples.
\end{enumerate}

Overall, these results validate our fine-tuning methodology across diverse knowledge graph types. For Wikidata, the dramatic improvements demonstrate that fine-tuning can overcome the challenges of numeric identifiers and massive scale. For knowledge graphs with richer dataset options, the consistent superiority of template-based fine-tuning across all scenarios suggests that this approach should be prioritized in future work on knowledge graph query generation.

\subsection{System Performance Comparison}
\label{ga-system-comparison}

Our comprehensive evaluation demonstrates the substantial advantages of our fine-tuning approach compared to FrOG's few-shot methodology across all knowledge graph domains. Table \ref{table-system-comparison} presents a comparison of the overall performance between our system and FrOG.
...
Several critical insights emerge from this comprehensive comparison:

First, our fine-tuned system consistently outperforms FrOG across all knowledge graph domains, with performance improvements ranging from $+4.9\%$ on Wikidata to $+33.3\%$ on GESIS KG. The largest improvements are observed on domain-specific knowledge graphs (Legal KG and GESIS KG), where our fine-tuning approach demonstrates particular effectiveness in learning specialized schemas and vocabularies. These improvements are achieved despite our system operating without any few-shot examples, while FrOG relies heavily on manually crafted few-shot examples for its operation.

Second, the performance gap between our system and FrOG increases significantly for knowledge graphs with more standardized schemas. The GESIS KG, which uses schema.org vocabularies, shows the largest improvement ($+33.3\%$), followed by the Legal KG ($+18.3\%$) and Curriculum KG ($+13.1\%$). This pattern suggests that our fine-tuning approach is particularly effective at learning standardized patterns and relationships that appear consistently across different query types.

Third, even on Wikidata, where FrOG was originally designed and optimized with established few-shot examples, our system achieves superior performance. This is particularly noteworthy given Wikidata's massive scale and complexity, which presents significant challenges for any query generation approach. The improvement, while modest ($+4.9\%$), demonstrates the robustness of our fine-tuning methodology.

\subsubsection{Training Cost vs. Inference Efficiency Trade-offs}
While our fine-tuning approach requires substantial computational resources during the training phase, it offers significant advantages during inference that offset these initial costs:

\begin{enumerate}
    \item \textbf{Training Resource Requirements}: Our fine-tuning process using QLoRA requires significant computational resources, typically taking 5-8 hours on P100 GPU used in Kaggle for each knowledge graph domain. This includes data preparation and model fine-tuning. The resource intensity of fine-tuning represents a one-time investment that enables improved performance across all subsequent queries.
    
    \item \textbf{Inference Efficiency Advantage}: FrOG's reliance on few-shot examples creates substantial inference overhead by requiring multiple example queries to be included in each input prompt. This increases input token length by 2-3x compared to our system, resulting in proportionally higher computational costs and latency for each query. Our fine-tuned system operates with minimal input tokens (just the natural language question), leading to faster inference times and reduced computational costs during deployment.
    
    \item \textbf{Scalability Considerations}: For production systems that process thousands of queries daily, our approach's inference efficiency provides substantial long-term cost savings despite the higher upfront training investment. The amortized cost per query decreases significantly as query volume increases, making our approach more economical for high-throughput applications.
    
    \item \textbf{Deployment Simplicity}: Our fine-tuned models eliminate the need for maintaining and managing few-shot example databases, reducing system complexity and potential points of failure. This simplification in the inference pipeline contributes to improved system reliability and easier maintenance.
\end{enumerate}

\subsubsection{Generalization and Adaptability}
Our experimental results demonstrate superior generalization capabilities compared to FrOG's few-shot approach:

\begin{enumerate}
    \item \textbf{Cross-Pattern Generalization}: Models fine-tuned on template datasets perform well on random-walk test sets, indicating effective generalization beyond the specific patterns seen during training. This suggests that our fine-tuning approach learns fundamental SPARQL generation principles rather than memorizing specific query templates.
    
    \item \textbf{Domain Transfer}: The consistent performance improvements across diverse knowledge graph domains (scholarly, legal, educational, and general knowledge) demonstrate the transferability of our fine-tuning methodology. This adaptability contrasts with FrOG's requirement for domain-specific few-shot example curation.
    
    \item \textbf{Schema Learning}: Our system effectively learns complex schema relationships and vocabularies through fine-tuning, eliminating the need for manually crafted examples that may not capture all relevant patterns within a domain. This automated learning capability is particularly valuable for large or rapidly evolving knowledge graphs.
\end{enumerate}

These findings establish clear evidence for the superiority of our fine-tuning approach over few-shot methodologies for knowledge graph query generation. While the initial training investment is substantial, the resulting improvements in inference efficiency, scalability, and performance make our approach more suitable for practical deployment scenarios, particularly for systems requiring high throughput or operating across multiple knowledge graph domains.

\section{Dataset Generation Methods}
\label{ga-dataset}
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{Curriculum KG} & $0.5542$ & $0.5580$ \\
    \RaggedRight{Legal KG} & $0.3810$ & $0.3672$ \\
    \RaggedRight{GESIS KG} & $0.4040$ & $0.3996$ \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
...
First, there is no consistent performance gap between template-generated and random-walk-generated test sets across knowledge graphs. The Curriculum KG shows nearly identical performance on both dataset types (template: $0.5542$, random-walk: $0.5580$), while the Legal KG ($-0.0138$) and GESIS KG ($-0.0044$) show slightly worse performance on random-walk datasets. This suggests that both dataset generation methods create valid and useful evaluation benchmarks.
...
\begin{enumerate}
    ...
    \item \textbf{Few-shot Example Selection}: FrOG's few-shot approach relies heavily on the quality and representativeness of the selected examples. Our results suggest that manually selecting few-shot examples may not capture the full diversity of query patterns, particularly for complex knowledge graphs.
\end{enumerate}
...
\begin{enumerate}
    ...
    \item For systems using few-shot learning (like FrOG), ensure that examples cover a diverse range of query patterns and structural relationships.
\end{enumerate}
