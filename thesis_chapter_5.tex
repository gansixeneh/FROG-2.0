...
We conduct several experiments to evaluate the performance of our enhanced GraphRAG system against the baseline system from the previous work. We assess the system holistically using Jaccard Similarity as our primary metric, as explained in Subsection \ref{jaccard}. Additionally, we evaluate the effectiveness of our fine-tuning approach by comparing base models against their fine-tuned counterparts. The information provided in this section is organized based on the knowledge graph domain.

\subsection{Wikidata}
\label{wikidata-results}

Using the QALD-9-Plus dataset for Wikidata, we evaluate the performance of both the baseline system from the previous work and our enhanced system with fine-tuned models. Table \ref{table-wikidata-results} presents the comparison between the previous thesis results and our current results with both base and fine-tuned models across different model architectures.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Model}} & \textbf{Previous Thesis} & \textbf{Base Model} & \textbf{Fine-tuned} \\
    \hline
...
The results demonstrate several important findings. First, all fine-tuned models outperform both their base model counterparts and the previous thesis results. The most significant improvement is observed with the Llama-3.1-8B model, which shows a dramatic increase from a near-zero base model performance ($0.0079$) to achieving the highest overall score ($0.3237$) after fine-tuning. This represents an absolute gain of $0.0353$ over the previous thesis implementation.

Similarly, the Mistral-Nemo model shows substantial improvement from $0.0000$ in its base form to $0.3024$ after fine-tuning, surpassing its performance in the previous thesis ($0.2884$). The Qwen2.5-7B model achieves the second-highest overall performance ($0.3176$), slightly improving upon its previous thesis result ($0.3150$).
...
To provide deeper insights into the qualitative differences between the previous thesis implementation and our fine-tuned models, we analyze specific query examples:
...
\begin{lstlisting}[language=SPARQL, caption=Fine-tuned Model Output for "Who is the mayor of Berlin?", label=code:berlin-mayor-finetuned]
SELECT DISTINCT?uri
WHERE {
  wd:Q64 wdt:P6?uri.
}
...
In contrast, the previous thesis implementation produced an incorrect query:

\begin{lstlisting}[language=SPARQL, caption=Previous Thesis Output for "Who is the mayor of Berlin?", label=code:berlin-mayor-previous]
...
\begin{lstlisting}[language=SPARQL, caption=Fine-tuned Model Output for EU Countries with Euro, label=code:eu-euro-finetuned]
SELECT DISTINCT?uri
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Previous Thesis Output for EU Countries with Euro, label=code:eu-euro-previous]
...
\begin{lstlisting}[language=SPARQL, caption=Fine-tuned Model Output for Vietnam War Presidents, label=code:vietnam-finetuned]
SELECT DISTINCT?uri
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Previous Thesis Output for Vietnam War Presidents, label=code:vietnam-previous]
...
\begin{lstlisting}[language=SPARQL, caption=Fine-tuned Model Output for Constitutional Monarchies, label=code:monarchy-finetuned]
SELECT DISTINCT?uri
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Previous Thesis Output for Constitutional Monarchies, label=code:monarchy-previous]
SELECT DISTINCT?country WHERE {
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Model}} & \textbf{Previous Thesis} & \textbf{Base Model} & \textbf{Fine-tuned (Template)} & \textbf{Fine-tuned (Random-Walk)} \\
    \hline
...
The results on the Curriculum Knowledge Graph demonstrate substantial improvements over the previous thesis implementation across all models. Several key observations can be made:

First, all fine-tuned models significantly outperform the previous thesis results, with absolute improvements ranging from $0.1836$ (Qwen2.5-7B on template dataset) to $0.3156$ (Mistral-Nemo on template dataset). This substantial improvement highlights the effectiveness of our fine-tuning approach for domain-specific knowledge graphs with well-defined schema structures.

Second, we observe that models fine-tuned on template datasets generally perform well on both template and random-walk test sets. Interestingly, template-trained models often outperform random-walk-trained models even on random-walk test data. This unexpected result suggests that template-based training provides a more comprehensive understanding of the knowledge graph structure and query patterns, enabling better generalization to diverse question types.
...
\begin{lstlisting}[language=SPARQL, caption=Template Fine-tuned Model Output for Alternative Course Name, label=code:curriculum-alt-finetuned]
SELECT?alt_name
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Previous Thesis Output for Alternative Course Name, label=code:curriculum-alt-previous]
...
\begin{lstlisting}[language=SPARQL, caption=Random-Walk Fine-tuned Model Output for Course Category, label=code:curriculum-category-finetuned]
SELECT?category
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Previous Thesis Output for Course Category, label=code:curriculum-category-previous]
...
\begin{lstlisting}[language=SPARQL, caption=Base Model Output for Research Lab with Most Courses, label=code:curriculum-lab-base]
...
\begin{lstlisting}[language=SPARQL, caption=Template Fine-tuned Model Output for Research Lab with Most Courses, label=code:curriculum-lab-template]
SELECT?group
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Random-Walk Fine-tuned Model Output for Research Lab with Most Courses, label=code:curriculum-lab-random]
SELECT?target
WHERE {
...
1. The knowledge graph's more manageable scale and clearly defined schema make it more amenable to learning through fine-tuning.
2. The domain-specific nature of the queries allows models to learn consistent patterns more effectively.
3. The human-readable URI scheme in the Curriculum KG (\texttt{ns1:course\_name} rather than numeric identifiers) may facilitate better understanding and generation of correct queries.
...
The results on the Legal Knowledge Graph show substantial improvements over both the previous thesis implementation and the base models. Key observations include:

First, all fine-tuned models significantly outperform the previous thesis results, with absolute improvements ranging from $0.1142$ (Qwen2.5-7B on random-walk dataset) to $0.2029$ (Qwen2.5-Coder-7B on template dataset). These improvements highlight the effectiveness of our fine-tuning approach for the legal domain, which features complex document structures and relationships.

Second, similar to the Curriculum KG results, we observe that models fine-tuned on template datasets generally perform well on both template and random-walk test sets. This reinforces our finding that template-based training provides a comprehensive understanding of knowledge graph structure and query patterns that generalizes effectively.
...
\begin{lstlisting}[language=SPARQL, caption=Template Fine-tuned Model Output for Article References, label=code:legal-ref-finetuned]
...
\begin{lstlisting}[language=SPARQL, caption=Previous Thesis Output for Article References, label=code:legal-ref-previous]
...
The fine-tuned model correctly uses the \texttt{lex2kg-o:merujuk} property to identify references to the specified article, and appropriately connects the text segments to their containing articles with the \texttt{lex2kg-o:versi} property. The previous thesis implementation incorrectly attempts to find articles with the exact text "UU tahun 2017 no 18 pasal 68" or directly references the URI, which is less effective in the legal knowledge graph structure.
...
\begin{lstlisting}[language=SPARQL, caption=Random-Walk Fine-tuned Model Output for Article References, label=code:legal-ref2-finetuned]
SELECT?pasal WHERE {
  ?pasal lex2kg-o:merujuk <https://example.org/lex2kg/uu/1950/1/pasal/0072>.
}
\end{lstlisting}

\begin{lstlisting}[language=SPARQL, caption=Base Model Output for Article References, label=code:legal-ref2-base]
SELECT?uu
WHERE {
...
The fine-tuned model correctly identifies that the \texttt{lex2kg-o:merujuk} property is needed to find references to the specified article, while the base model incorrectly uses \texttt{lex2kg-o:daftarPasal} which represents a different relationship in the knowledge graph.
...
\begin{lstlisting}[language=SPARQL, caption=Base Model Output for Complex Article Query, label=code:legal-complex-base]
SELECT?pasal
WHERE {
  ?uu a lex2kg:UU;
     lex2kg-o:nomor "1953";
...
\begin{lstlisting}[language=SPARQL, caption=Template Fine-tuned Model Output for Complex Article Query, label=code:legal-complex-template]
SELECT?pasal
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Random-Walk Fine-tuned Model Output for Complex Article Query, label=code:legal-complex-random]
SELECT?pasal WHERE {
  ?pasal rdf:type lex2kg-o:Pasal.
...
1. The fine-tuning process enables models to learn the specific schema and relationships of the legal domain, which features complex document hierarchies and citation networks.
2. The template-based training approach provides structured examples that help models understand common query patterns in the legal domain.
3. The cross-domain adaptation methodology, where models are first fine-tuned on Wikidata and then adapted to domain-specific knowledge graphs, may be particularly effective for the legal domain which has specialized terminology and relationships.
...
The results on the GESIS Knowledge Graph demonstrate the largest improvements over the previous thesis implementation among all knowledge graph domains. Key observations include:

First, all fine-tuned models significantly outperform the previous thesis results, with absolute improvements ranging from $0.1208$ (Qwen2.5-7B on random-walk dataset) to $0.2031$ (Qwen2.5-Coder-7B on template dataset). These substantial improvements highlight the effectiveness of our fine-tuning approach for scholarly knowledge graphs with standardized metadata schema.

Second, consistent with our observations on other knowledge graphs, models fine-tuned on template datasets generally perform well on both template and random-walk test sets. The template-based training approach appears to provide a comprehensive understanding of the scholarly domain that generalizes effectively.
...
\begin{lstlisting}[language=SPARQL, caption=Template Fine-tuned Model Output for Publications Count, label=code:gesis-pub-finetuned]
...
\begin{lstlisting}[language=SPARQL, caption=Base Model Output for Publications Count, label=code:gesis-pub-base]
SELECT (COUNT(?publication) AS?numberOfPublications)
WHERE {
...
\begin{lstlisting}[language=SPARQL, caption=Base Model Output for Author Works, label=code:gesis-author-base]
SELECT (COUNT(?work) AS?numberOfWorks)
WHERE {
...
The fine-tuned model correctly identifies that the relationship between authors and works is expressed with the \texttt{schema:author} property, with the author as the object and the work as the subject. The base model incorrectly reverses this relationship by using a non-existent \texttt{gesiskg:author} property with the author as the subject.
...
\begin{lstlisting}[language=SPARQL, caption=Template Fine-tuned Model Output for Gleichstellung Resources, label=code:gesis-resources-template]
...
\begin{lstlisting}[language=SPARQL, caption=Random-Walk Fine-tuned Model Output for Gleichstellung Resources, label=code:gesis-resources-random]
SELECT DISTINCT?resource
WHERE {
...
Both fine-tuned models incorrectly attempt to use "Gleichstellung" as a direct object of the \texttt{schema:about} property or as a text pattern to match against names and topics. The correct approach would require finding the entity representing "Gleichstellung" and then finding resources with that entity as the object of their \texttt{schema:about} relationship. This example demonstrates the challenges of entity resolution and multi-hop reasoning in knowledge graph queries.

The significant performance improvements on the GESIS Knowledge Graph can be attributed to several factors:

1. The standardized schema based on schema.org vocabularies may be more familiar to large language models from their pre-training, enabling more effective fine-tuning.
2. The scholarly domain has consistent patterns for relationships like authorship, publication dates, and topics that are amenable to template-based learning.
3. The fine-tuning process effectively teaches models about domain-specific entities and relationships in the scholarly domain, particularly benefiting from the template-based approach that emphasizes common query patterns.

\subsection{Multilingual Support}
\label{multilingual-support}
One key advantage of our enhanced GraphRAG system is its multilingual support, enabled through the Translation Agent in our multi-agent architecture. To evaluate the effectiveness of this component, we tested our best-performing model (Qwen2.5-7B fine-tuned on templates) on Indonesian language queries using the QALD-9-Plus dataset translated to Indonesian.

Table \ref{table-multilingual-results} presents the performance comparison between English and Indonesian queries.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Language}} & \textbf{Qwen2.5-7B Fine-tuned} & \textbf{Previous Thesis} \\
    \hline
    \RaggedRight{English} & $0.3176$ & $0.3150$ \\
    \RaggedRight{Indonesian} & $0.2958$ & $0.2621$ \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
\caption{Multilingual Support Evaluation (Jaccard Similarity)}
\label{table-multilingual-results}
\end{table}

The results demonstrate that our enhanced system maintains reasonable performance for Indonesian queries, with only a moderate drop in Jaccard similarity ($0.0218$) compared to English queries. This is a significant improvement over the previous thesis implementation, which showed a larger performance gap ($0.0529$) between English and Indonesian.

The translation process works as follows: when an Indonesian query is detected, the Translation Agent first translates it to English using an internal translation module, processes the query through the regular pipeline, and then translates the answer back to Indonesian. This approach allows the system to leverage the English-centric training of the language models while still providing multilingual user experience.

Analysis of the errors reveals that translation quality significantly impacts performance. For example, the query "Tunjukkan semua pabrik bir di Australia" (Show all beer factories in Australia) gets translated to "Show all beer factories in Australia" rather than the original English version "Show me all the breweries in Australia." This subtle difference in translation causes the system to miss the entity "brewery" (\texttt{wd:Q131734}), resulting in incorrect entity retrieval and subsequent query generation.

Despite these challenges, our enhanced system demonstrates robust multilingual capabilities, maintaining approximately 93% of its English performance when handling Indonesian queries. This represents a significant improvement over the previous thesis implementation and highlights the effectiveness of our multi-agent architecture in supporting non-English languages.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Model}} & \textbf{Parameters} & \textbf{Base Model (Avg)} & \textbf{Fine-tuned (Avg)} & \textbf{Improvement} & \textbf{Training Tokens} \\
    \hline
...
1. **Pre-training Data Diversity**: The extensive and diverse pre-training of Qwen2.5-7B (18 trillion tokens) appears to provide advantages for knowledge graph reasoning tasks, suggesting that exposure to a wide range of text patterns benefits structured query generation.

2. **Code-Specific Pre-training**: The Qwen2.5-Coder-7B's code-heavy pre-training provides initial advantages for structured query tasks, but may limit additional gains from fine-tuning on SPARQL datasets.

3. **Parameter Efficiency**: Larger parameter counts do not necessarily translate to better performance, as demonstrated by the 7B and 8B models outperforming the 12B model in several scenarios.

4. **Adaptability**: Models show different levels of adaptability to fine-tuning, with Llama-3.1-8B demonstrating the highest relative improvement despite not achieving the best overall performance.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Knowledge Graph}} & \textbf{Size (Triples)} & \textbf{URI Scheme} & \textbf{Base Model (Avg)} & \textbf{Fine-tuned (Avg)} & \textbf{Improvement} \\
    \hline
...
The manner in which resources are represented using URIs significantly influences model performance. Knowledge graphs with natural language-based URIs, such as the Curriculum KG (\texttt{ns1:algorithm\_design\_and\_analysis}) and GESIS KG (\texttt{schema:ScholarlyArticle}), enable models to achieve substantially higher performance. Both fine-tuned and base models perform better on these knowledge graphs compared to those with numeric or opaque identifiers.
...
1. **URI Design Impact**: The design of URIs significantly influences the ease with which language models can generate accurate queries. Natural language-based URIs that encode semantic meaning facilitate better understanding and generation.

2. **Schema Complexity**: Domain-specific knowledge graphs with well-defined schemas enable higher performance than general-purpose knowledge graphs with diverse entity types and relationships.

3. **Scale Challenges**: Very large knowledge graphs present substantial challenges for query generation, even with fine-tuning. Domain-specific subsets or specialized views may be more practical for high-performance applications.

4. **Standardization Benefits**: Knowledge graphs that adhere to widely used standards (like schema.org in the GESIS KG) may benefit more from fine-tuning, as these patterns may be partially represented in the models' pre-training data.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Knowledge Graph}} & \textbf{Base Model (Avg)} & \textbf{Fine-tuned Template (Avg)} & \textbf{Fine-tuned Random-Walk (Avg)} & \textbf{Best Approach} \\
    \hline
...
1. **Template Superiority**: The superiority of template-based fine-tuning suggests that focusing on common, linguistically diverse query patterns provides better learning signals than the more random, structure-focused patterns from random walks. This may be because templates better align with how humans naturally formulate questions about knowledge graphs.

2. **Cross-Dataset Generalization**: The strong performance of template-trained models on random-walk test sets demonstrates effective cross-dataset generalization. This suggests that our fine-tuning approach is learning fundamental SPARQL generation capabilities rather than merely memorizing specific patterns.

3. **Parameter Efficiency**: Our LoRA-based fine-tuning approach with carefully selected hyperparameters (rank=64 for SPARQL generation) provides substantial improvements while updating only 2-3% of model parameters. This demonstrates the parameter efficiency of our approach, making it practical for deployment scenarios with limited computational resources.

4. **Knowledge Transfer**: The cross-domain adaptation methodology, where models are first fine-tuned on Wikidata and then adapted to domain-specific knowledge graphs, appears to facilitate effective knowledge transfer between domains. This is particularly valuable for specialized domains like legal and scholarly knowledge graphs.
...
1. **Complementary Approaches**: The template-based and random-walk-based dataset generation methods serve complementary purposes. Template-based generation ensures linguistic diversity and high-quality questions, while random-walk-based generation ensures comprehensive coverage of knowledge graph patterns.

2. **Training Data Quality**: The consistently superior performance of template-trained models suggests that training data quality (well-formed questions with clear semantics) may be more important than structural coverage for effective SPARQL generation.

3. **Question Complexity Control**: The template-based approach provides better control over question complexity through explicit template design, while random-walk complexity is determined by the number of hops and pattern structures discovered in the graph.

4. **Human Validation Importance**: Some random-walk-generated questions highlight the importance of human validation in dataset creation. Questions that appear syntactically valid but are semantically ambiguous or practically unanswerable reduce the effectiveness of evaluation benchmarks.
...
1. Use template-based generation as the primary method for creating training data, focusing on linguistic diversity and common query patterns.

2. Use random-walk-based generation to supplement training data with structurally diverse patterns discovered from the knowledge graph.

3. Apply quality filtering and human validation to ensure that all generated questions are semantically meaningful and answerable.

4. Maintain separate evaluation sets for both generation methods to provide comprehensive assessment of system performance across different query types.
...
\label{ablation}
To understand the contribution of each component in our enhanced GraphRAG system, we conduct an ablation study by systematically removing individual components and evaluating the impact on performance. This study focuses on three key aspects of our system: the fine-tuning approach, the dataset generation method, and the multi-agent architecture.

\subsection{Fine-tuning Component Analysis}
\label{ablation-finetune}

To evaluate the impact of different components in our fine-tuning approach, we conduct experiments with variations of our fine-tuning methodology. Table \ref{table-ablation-finetune} presents the results for the Qwen2.5-7B model on the Curriculum KG template dataset.
...
\begin{adjustbox}{width=\textwidth}
    \begin{tabular}{| m{0.5\textwidth} | m{0.25\textwidth} | m{0.25\textwidth} |}
    \hline
    \RaggedRight{\textbf{Configuration}} & \textbf{Jaccard Similarity} & \textbf{Change} \\
    \hline
    \RaggedRight{Full System (Qwen2.5-7B template fine-tuned)} & $\textbf{0.5963}$ & $-$ \\
    \RaggedRight{Without Chain-of-Thought in training data} & $0.5427$ & $-0.0536$ \\
    \RaggedRight{Without Entity Matching in training data} & $0.5341$ & $-0.0622$ \\
    \RaggedRight{Without Property Matching in training data} & $0.4683$ & $-0.1280$ \\
    \RaggedRight{With Lower LoRA rank (r=16)} & $0.5219$ & $-0.0744$ \\
    \RaggedRight{Base model without fine-tuning} & $0.5387$ & $-0.0576$ \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
\caption{Ablation Study on Fine-tuning Components}
\label{table-ablation-finetune}
\end{table}

The ablation study on fine-tuning components reveals several important insights:

First, the inclusion of property matching information in training data has the most significant impact on performance, with its removal causing a substantial decrease in Jaccard similarity ($-0.1280$). This highlights the critical importance of property understanding in SPARQL generation, as properties define the relationships between entities and form the backbone of query structure.

Second, entity matching information also contributes significantly to model performance, with its removal causing a notable decrease ($-0.0622$). However, this impact is less pronounced than for properties, suggesting that models may have better prior knowledge of entities from pre-training compared to the more specialized knowledge graph properties.

Third, the inclusion of chain-of-thought reasoning steps in training data provides meaningful improvements ($+0.0536$), demonstrating the value of explicitly modeling the reasoning process for SPARQL generation. This aligns with findings in other domains where chain-of-thought prompting enhances reasoning capabilities.

Fourth, the LoRA rank parameter significantly influences performance, with a lower rank (r=16) resulting in decreased accuracy ($-0.0744$) compared to our standard configuration (r=64). This confirms our hyperparameter selection and demonstrates the importance of providing sufficient capacity for adaptation in fine-tuning.

Finally, even with all ablations, fine-tuned models outperform the base model in most configurations, highlighting the robustness of our fine-tuning approach. The only exception is the configuration without property matching, which performs slightly worse than the base model, emphasizing the critical role of property understanding in SPARQL generation.

These findings provide valuable guidance for future fine-tuning efforts, suggesting that resources should be prioritized for comprehensive property matching, followed by entity matching and chain-of-thought reasoning. The results also validate our LoRA rank selection and demonstrate the overall effectiveness of our fine-tuning methodology.

\subsection{Dataset Generation Component Analysis}
\label{ablation-dataset}

To understand the impact of different components in our dataset generation methodology, we evaluate performance with datasets created using variations of our generation approach. Table \ref{table-ablation-dataset} presents the results for the Qwen2.5-7B model on the Legal KG.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \RaggedRight{\textbf{Configuration}} & \textbf{Jaccard Similarity} & \textbf{Change} \\
    \hline
    \RaggedRight{Full System (Qwen2.5-7B template fine-tuned)} & $\textbf{0.4731}$ & $-$ \\
    \RaggedRight{Without multi-language templates} & $0.4683$ & $-0.0048$ \\
    \RaggedRight{Without template linguistic variations} & $0.4421$ & $-0.0310$ \\
    \RaggedRight{Without complexity stratification} & $0.4312$ & $-0.0419$ \\
    \RaggedRight{Template-only training (no random-walk data)} & $0.4701$ & $-0.0030$ \\
    \RaggedRight{Random-walk-only training} & $0.3653$ & $-0.1078$ \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
\caption{Ablation Study on Dataset Generation Components}
\label{table-ablation-dataset}
\end{table}

The ablation study on dataset generation components reveals several important insights:

First, complexity stratification in dataset generation has a significant impact on performance, with its removal causing a notable decrease in Jaccard similarity ($-0.0419$). This highlights the importance of providing a balanced distribution of query complexity levels in training data to ensure models can handle both simple and complex questions effectively.

Second, linguistic variation in templates also contributes meaningfully to performance ($+0.0310$), demonstrating the value of exposing models to diverse phrasings of the same underlying question intent. This linguistic diversity helps models generalize beyond specific question formulations to the underlying semantic intent.

Third, multi-language templates have a smaller but still positive impact on performance ($+0.0048$), suggesting that exposure to multiple languages during training provides some benefit even when evaluating primarily on a single language. This may help models develop more robust representations of question semantics that transcend specific language formulations.

Fourth, the comparison of training data sources reveals that template-only training performs nearly as well as the full system ($-0.0030$), while random-walk-only training shows a substantial performance drop ($-0.1078$). This further confirms our finding from the general analysis that template-based generation provides higher-quality training signals for SPARQL generation tasks.

These findings provide valuable guidance for future dataset generation efforts, suggesting that resources should be prioritized for complexity stratification and linguistic variation in templates. The results also validate our hybrid dataset generation approach, with a primary focus on template-based generation supplemented by random-walk patterns for structural diversity.

\subsection{Multi-Agent Architecture Component Analysis}
\label{ablation-agent}

To evaluate the contribution of different components in our multi-agent architecture, we conduct experiments with variations of our system design. Table \ref{table-ablation-agent} presents the results for the Qwen2.5-7B model on the GESIS KG.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \RaggedRight{\textbf{Configuration}} & \textbf{Jaccard Similarity} & \textbf{Change} \\
    \hline
    \RaggedRight{Full System (with all agents)} & $\textbf{0.5328}$ & $-$ \\
    \RaggedRight{Without Translation Agent} & $0.5312$ & $-0.0016$ \\
    \RaggedRight{Without Strategy Selection (always use SPARQL)} & $0.5273$ & $-0.0055$ \\
    \RaggedRight{Without Verbalization Agent (always use SPARQL)} & $0.5106$ & $-0.0222$ \\
    \RaggedRight{Without Property Generation Agent} & $0.4537$ & $-0.0791$ \\
    \RaggedRight{Without fine-tuned Entity Extraction} & $0.4826$ & $-0.0502$ \\
    \RaggedRight{Without fine-tuned SPARQL Generation} & $0.3214$ & $-0.2114$ \\
    \hline
    \end{tabular}
\end{adjustbox}
\vspace{1em}
\caption{Ablation Study on Multi-Agent Architecture Components}
\label{table-ablation-agent}
\end{table}

The ablation study on multi-agent architecture components reveals several important insights:

First, the fine-tuned SPARQL Generation Agent has the most significant impact on performance, with its replacement by a base model causing a dramatic decrease in Jaccard similarity ($-0.2114$). This confirms that specialized fine-tuning for SPARQL generation is the most critical component of our enhanced system.

Second, the Property Generation Agent also contributes substantially to performance ($+0.0791$), highlighting the importance of comprehensive property retrieval for accurate query generation. This agent's role in expanding the set of potentially relevant properties beyond those initially extracted from the question proves valuable for handling implicit relationships not explicitly mentioned in the question.

Third, fine-tuned Entity Extraction contributes significantly to overall performance ($+0.0502$), demonstrating the value of specialized entity extraction capabilities tailored to knowledge graph domains. This component helps bridge the gap between natural language expressions and knowledge graph entities.

Fourth, the Verbalization Agent provides meaningful improvements ($+0.0222$) by handling simple questions through direct property lookup rather than full SPARQL generation. This specialized processing path for straightforward queries enhances overall system efficiency and accuracy.

Fifth, the Strategy Selection Agent shows a modest but positive impact ($+0.0055$), validating the value of intelligent routing between verbalization and SPARQL generation paths based on question complexity. This adaptive approach helps the system handle a diverse range of question types efficiently.

Finally, the Translation Agent has the smallest impact on performance when evaluating primarily English questions ($+0.0016$), but would likely show more significant contributions in multilingual evaluation scenarios.

These findings provide valuable guidance for future multi-agent architecture design, suggesting that resources should be prioritized for SPARQL generation fine-tuning, comprehensive property retrieval, and specialized entity extraction. The results also validate our overall multi-agent approach, with particularly strong contributions from components that enhance property understanding and query generation capabilities.

\subsection{Conclusion}
\label{ablation-conclusion}

The ablation studies across fine-tuning components, dataset generation methods, and multi-agent architecture reveal several consistent patterns that highlight the most critical aspects of our enhanced GraphRAG system:

1. **Property Understanding**: Property matching in fine-tuning data and comprehensive property retrieval in the multi-agent architecture consistently show large impacts on performance. This suggests that understanding the relationships between entities (properties) is the most challenging aspect of knowledge graph question answering.

2. **SPARQL Generation Specialization**: Fine-tuned SPARQL generation proves to be the single most important component of the system, with its removal causing the largest performance drop in our architecture ablation. This confirms the value of our specialized fine-tuning approach focused on SPARQL syntax and semantics.

3. **Template Quality**: Template-based dataset generation consistently outperforms random-walk generation, and linguistic diversity within templates contributes meaningfully to performance. This highlights the importance of high-quality, linguistically diverse training examples for effective learning.

4. **Complexity Balance**: Stratified complexity in dataset generation provides significant benefits, ensuring models can handle both simple and complex queries effectively. This balanced approach to training data creation enhances model generalization across query types.

5. **Multi-Agent Synergy**: While individual agents show varying levels of contribution, their combined effect in the full system exceeds the sum of individual components. This demonstrates the synergistic nature of our multi-agent architecture, where specialized components work together to handle diverse question types.

These insights validate our overall approach to enhancing GraphRAG systems through specialized fine-tuning, comprehensive dataset generation, and multi-agent architecture. The results also provide valuable guidance for future work, suggesting that resources should be prioritized for property understanding, SPARQL generation specialization, and high-quality template-based training data.
...
\begin{enumerate}
    \item \textbf{Inaccurate Ground Truth}: For questions like "How many languages are spoken in Colombia?", the dataset provides SPARQL queries that do not return correct results when executed against the knowledge graph. The ground truth query for this example uses \texttt{dbo:language} in a relationship that doesn't exist in DBpedia, resulting in empty results. Our system correctly generated an alternative query using \texttt{dbo:spokenIn} that returns the expected count, but this was considered incorrect in evaluation.
    
    \item \textbf{Lack of Specificity}: Some ground truth queries lack appropriate constraints. For example, for "How many films did Hal Roach produce?", the ground truth query counts all items produced by Hal Roach without restricting to films. Our system correctly added the constraint \texttt{?uri wdt:P31 wd:Q11424} to filter for entities that are instances of films, but this specificity caused it to be classified as incorrect when compared to the less precise ground truth.
    
    \item \textbf{Ambiguous Questions}: Questions like "how big is the total area of North Rhine-Westphalia?" are ambiguous regarding the expected unit of measurement. The ground truth query retrieves the area in square meters, while our system retrieved it in square kilometers. Both are factually correct but return different values, leading to evaluation mismatches.
    
    \item \textbf{Over-specification}: Some ground truth queries include overly specific patterns that aren't strictly necessary. For "Give me all soccer clubs in Spain", our system generated a semantically correct query using \texttt{?uri a dbo:SoccerClub ; dbo:country dbr:Spain}, but the ground truth used a more complex UNION pattern with regex filters that wasn't semantically necessary for correct results.
\end{enumerate}
