...
We conduct several experiments to evaluate the performance of our enhanced GraphRAG system against the baseline system from the previous work, FrOG. We assess the system holistically using Jaccard Similarity as our primary metric. Additionally, we evaluate the effectiveness of our fine-tuning approach by comparing our system with base models against our system with fine-tuned counterparts. The information provided in this section is organized based on the knowledge graph domain.

\subsection{Wikidata}
\label{wikidata-results}

Using the QALD-9-Plus dataset for Wikidata, we evaluate the performance of both the baseline system (FrOG) from the previous work and our enhanced system with both base and fine-tuned models. It is important to note that while FrOG's original evaluation in their paper was conducted on a downsampled QALD-9-Plus train dataset, our evaluation (including our reimplementation of FrOG) is performed on the QALD-9-Plus test dataset for a fair comparison. Additionally, while FrOG's original evaluation used full models, our evaluation (including FrOG) uses quantized models due to resource constraints. Table \ref{table-wikidata-results} presents the comparison between FrOG results and our current results with both base and fine-tuned models across different model architectures.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Model}} & \textbf{FrOG (Few-shot)} & \textbf{Our System (Base Model)} & \textbf{Our System (Fine-tuned)} \\
    \hline
...
For the Curriculum Knowledge Graph, we evaluate performance using both template-based and random-walk datasets. It is important to note that FrOG was originally evaluated on Curriculum KG in their paper, so they had established few-shot examples for this knowledge graph. In our evaluation, we implemented FrOG using only the Qwen/Qwen2.5-7B-Instruct model due to time constraints, and we maintained their few-shot approach. In contrast, our system---both with base and fine-tuned models---does not use any few-shot examples. Table \ref{table-curriculum-template-results} presents the results on the template dataset, while Table \ref{table-curriculum-random-results} shows the results on the random-walk dataset.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Model}} & \textbf{FrOG (Few-shot)} & \textbf{Our System (Base Model)} & \textbf{Our System (Fine-tuned Template)} & \textbf{Our System (Fine-tuned Random-Walk)} \\
    \hline
...
First, our fine-tuned models significantly outperform both FrOG and our base models, with absolute improvements over FrOG ranging from $0.0536$ (Qwen2.5-7B on template dataset) to $0.1704$ (Llama-3.1-8B with random-walk fine-tuning on template dataset). This substantial improvement highlights the effectiveness of our fine-tuning approach for domain-specific knowledge graphs with well-defined schema structures.
...
First, our fine-tuned models significantly outperform FrOG, with absolute improvements ranging from $0.0715$ (Qwen2.5-7B fine-tuned with random-walk on random-walk dataset) to $0.1244$ (Qwen2.5-7B fine-tuned with template on template dataset). These improvements highlight the effectiveness of our fine-tuning approach for the legal domain, which features complex document structures and relationships, even without the benefit of few-shot examples that FrOG utilizes.
...
First, our fine-tuned models significantly outperform FrOG, with absolute improvements ranging from $0.0830$ (Qwen2.5-7B fine-tuned with random-walk on random-walk dataset) to $0.1549$ (Qwen2.5-Coder-7B fine-tuned with template on template dataset). These substantial improvements highlight the effectiveness of our fine-tuning approach for scholarly knowledge graphs with standardized metadata schema, even without the benefit of few-shot examples.
...
\begin{adjustbox}{width=\textwidth}
    ...
    \hline
    \RaggedRight{\textbf{Knowledge Graph}} & \textbf{Our System (Base Model Avg)} & \textbf{Our System (Fine-tuned Template Avg)} & \textbf{Our System (Fine-tuned Random-Walk Avg)} & \textbf{Best Approach} \\
    \hline
...
First, both fine-tuning approaches substantially improve performance over our system with base models across all knowledge graphs and datasets. The average improvement across all scenarios is $+0.1944$ for template-based fine-tuning and $+0.1655$ for random-walk-based fine-tuning. This confirms the effectiveness of our overall fine-tuning methodology, regardless of the specific dataset generation approach used. Importantly, these improvements are achieved without requiring few-shot examples, in contrast to FrOG.
