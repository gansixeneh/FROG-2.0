...
We conduct several experiments to evaluate the performance of our enhanced GraphRAG system against the baseline system from the previous work, FrOG. We assess the system holistically using Jaccard Similarity as our primary metric. Additionally, we evaluate the effectiveness of our fine-tuning approach by comparing our system with base models against our system with fine-tuned counterparts. It is important to note that while FrOG's original evaluation in their paper used full models, our evaluation (including our reimplementation of FrOG) uses quantized models due to resource constraints. Furthermore, FrOG relies on few-shot examples for its operation, while our system---both with base and fine-tuned models---does not use any few-shot prompting. The information provided in this section is organized based on the knowledge graph domain.
