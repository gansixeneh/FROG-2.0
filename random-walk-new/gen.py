...
Case 3 (3 properties): O—X—X—?, O—X—?—O, X branches to O,X,?
"""
...
class PatternBasedSPARQLGenerator:
    ...
    def _extract_properties(self):
        ...
        for s, p, o in self.graph:
            if isinstance(p, URIRef) and p not in skip_props:
                properties.add(p)
    ...
    def generate_3_property_patterns(self, count=100):
        ...
        - X branches to O, X, ? (star pattern)
    ...
    def _generate_star_pattern(self, prop1, prop2, prop3, props_str):
        """Generate star pattern: ?hidden branches to fixed_entity, ?hidden2, ?target"""
        patterns = []
        
        # Find a fixed entity for one branch
        entities = list(self.prop_to_objects[prop1])
        if not entities:
            return patterns
        ...
        for i in range(8):
            ...
            if i & 1:  # bit 0
                pattern_parts.append(f"{fixed_str} {props_str[0]} ?hidden")
            else:
                pattern_parts.append(f"?hidden {props_str[0]} {fixed_str}")
                
            if i & 2:  # bit 1
                pattern_parts.append(f"?hidden2 {props_str[1]} ?hidden")
            else:
                pattern_parts.append(f"?hidden {props_str[1]} ?hidden2")
...
def main():
    ...
    dataset = generator.generate_dataset(size=150)
