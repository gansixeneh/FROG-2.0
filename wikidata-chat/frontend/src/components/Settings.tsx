        ...
        <div className="space-y-6">
          ...
          <div className="flex items-start justify-between p-4 bg-gray-50 rounded-lg">
            <div className="flex-1 pr-4">
              ...
              <p className="text-sm text-gray-500 mb-3">
                When enabled, FrOG will use Google Search as a fallback when Wikidata methods fail to provide sufficient results. 
                When disabled, FrOG will only use Wikidata sources.
              </p>
              <span className={`inline-block px-3 py-1 rounded-full text-xs font-medium ${
                settings.useGoogleSearch ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
              }`}>
                {settings.useGoogleSearch ? 'Enabled: Uses Google Search as fallback' : 'Disabled: Wikidata only'}
              </span>
            </div>
            <div className="flex-shrink-0">
              <button
